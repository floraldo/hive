...
def check_ruff_configuration():
    ...
    if ecosystemiser_pyproject.exists():
        ...
        with open(ecosystemiser_pyproject, 'r') as f:
            content = f.read()
...
def check_active_processes():
    ...
    result = subprocess.run("ps aux | grep -i 'visual studio code' | grep -v grep", shell=True, capture_output=True, text=True)
    if result.stdout:
    ...
    result = subprocess.run("ps aux | grep python | grep -E '(format|lint|ruff)' | grep -v grep", shell=True, capture_output=True, text=True)
    if result.stdout:
        ...
        for i in range(5):
            ...
            if current_mtime != initial_mtime:
                ...
                if test_comment in current_content:
                    print(f"  ✅ Our modification is still present")
                else:
                    print(f"  ❌ Our modification was REVERTED!")
