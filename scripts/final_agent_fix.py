    ...
    for i, line in enumerate(lines):
        ...
        if (
            ...
            and (
                ...
                or line.rstrip().endswith('"')
                or line.rstrip().endswith("'")
                or re.search(r"\w+$", line.rstrip())
            )
            and re.match(r"\s+\w+=", next_line)
        ):
            needs_comma = True
        ...
        if (
            not line.rstrip().endswith(",")
            and re.search(r'\w+\["[\w_]+"\]$', line.rstrip())
            and re.match(r"\s+\w+=", next_line)
        ):
            needs_comma = True
        ...
        if (
            not line.rstrip().endswith(",")
            and re.search(r'sub_task\["[\w_]+"\]$', line.rstrip())
            and (re.match(r'\s+"[\w_]+":', next_line) or re.match(r"\s+}", next_line))
        ):
            needs_comma = True
