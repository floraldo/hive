...
class ValidationCache:
    ...
    def cache_result(
        self, file_path: Path, rule_name: str, passed: bool, violations: list[str]
    ) -> None:
        """
    ...
    def clear_cache(self, older_than_days: int | None = None) -> int:
        ...
        if older_than_days is None:
            ...
            cursor = conn.execute(
                "DELETE FROM validation_results WHERE timestamp < ?", (cutoff,)
            )
    ...
    def get_stats(self) -> dict:
        ...
        cursor = conn.execute(
            "SELECT MIN(timestamp), MAX(timestamp) FROM validation_results"
        )
