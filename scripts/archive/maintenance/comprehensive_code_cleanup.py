...
"""
        ...
        for category, issues in sorted(by_category.items()):
            sorted_issues = sorted(
                issues, key=lambda x: (severity_order.get(x.severity, 3), x.file_path, x.line_number)
            )
...
def main():
    ...
    parser.add_argument(
        "--category", help="Filter by category (technical_debt, security, async, imports, global_state)"
    )
    ...
    if not args.report and not args.json:
        ...
        print(
            f"- Categories: {', '.join(k for k in stats.keys() if not k.endswith('_high') and not k.endswith('_medium') and not k.endswith('_low') and k not in ['total', 'files_analyzed'])}"
        )
