...
class ComprehensiveCleanup:
    def __init__(self, project_root: Path = None, dry_run: bool = True):
        ...
        self.changes = []
        self.stats = {"files_moved": 0, "files_deleted": 0, "directories_created": 0, "directories_removed": 0}

    def log_change(self, action: str, source: str, target: str = None):
        """Log a change for reporting"""
        change = {"action": action, "source": source, "target": target, "timestamp": datetime.now()}
    ...
    def analyze_documentation_relevance(self, docs: Dict[str, List[Path]]) -> Dict[str, Dict[str, List[Path]]]:
        ...
        for category, files in docs.items():
            analysis[category] = {"keep_current": [], "archive": [], "delete": [], "reorganize": []}
    ...
    def should_reorganize(self, filename: str, content: str) -> bool:
        ...
        reorganize_indicators = ["architecture", "design", "api", "integration", "deployment", "setup", "configuration"]

        return any(indicator in filename or indicator in content.lower() for indicator in reorganize_indicators)
    ...
    def determine_new_location(self, file_path: Path) -> Path:
        ...
        if any(
            word in filename or word in content_sample.lower() for word in ["architecture", "design", "adr", "pattern"]
        ):
