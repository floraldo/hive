...
class ScriptsAnalyzer:
    ...
    def analyze_script(self, script_path: Path) -> ScriptMetadata:
        ...
        try:
            ...
            # Count TODOs
            todo_count = len([line for line in lines if any(marker in line.upper() 
    ...
    def identify_redundancy_groups(self) -> List[RedundancyGroup]:
        ...
        for group_def in group_definitions:
            ...
            for path, metadata in self.metadata_map.items():
                ...
                if any(keyword in script_name_lower or keyword in purpose_lower 
                       for keyword in group_def['keywords']):
...
"""
        ...
        for path in sorted(self.metadata_map.keys()):
            ...
            report += f"| {meta.name} | {meta.execution_type} | {meta.size_lines} | {meta.purpose[:50]}... | {deps_str} |\n"
...
def main():
    ...
    print(f"   - Potential reduction: {sum(len(g.redundant_scripts) for g in redundancy_groups) / len(metadata_map) * 100:.1f}%")
    
    print(f"\nNext Steps:")
    print(f"   1. Review audit report: {report_path}")
    print(f"   2. Run Phase 2: Consolidation planning")
    print(f"   3. Execute dry-run before making changes")
