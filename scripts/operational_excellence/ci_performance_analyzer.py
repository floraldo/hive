...
class CIPerfomanceAnalyzer:
    ...
    def analyze_job_bottlenecks(self, sample_runs: int = 20) -> None:
        ...
        self.analysis_results['job_bottlenecks'].sort(
            key=lambda x: x['avg_duration_minutes'], reverse=True
        )
    ...
    def generate_optimization_recommendations(self) -> None:
        ...
        slow_workflows = [w for w in self.analysis_results['workflow_performance'] 
                         if w['avg_duration_minutes'] > 10]
        ...
        bottleneck_jobs = [j for j in self.analysis_results['job_bottlenecks'] 
                          if j['is_bottleneck']]
    ...
    def generate_report(self) -> str:
        ...
        slow_workflows = len([w for w in self.analysis_results['workflow_performance'] 
                             if w['avg_duration_minutes'] > 10])
        bottleneck_jobs = len([j for j in self.analysis_results['job_bottlenecks'] 
                              if j['is_bottleneck']])
        ...
        if self.analysis_results['workflow_performance']:
            ...
            slowest = sorted(self.analysis_results['workflow_performance'], 
                           key=lambda x: x['avg_duration_minutes'], reverse=True)[:5]
            ...
            for workflow in slowest:
                status_emoji = "🔴" if workflow['avg_duration_minutes'] > 20 else "🟡" if workflow['avg_duration_minutes'] > 10 else "🟢"
                report_lines.append(
        ...
        if self.analysis_results['job_bottlenecks']:
            ...
            for job in self.analysis_results['job_bottlenecks'][:5]:
                status_emoji = "🔴" if job['avg_duration_minutes'] > 15 else "🟡" if job['avg_duration_minutes'] > 5 else "🟢"
                report_lines.append(
        ...
        if self.analysis_results['optimization_recommendations']:
            ...
            # Group by priority
            high_priority = [r for r in self.analysis_results['optimization_recommendations'] if r['priority'] == 'high']
            medium_priority = [r for r in self.analysis_results['optimization_recommendations'] if r['priority'] == 'medium']
        ...
        # Cost Estimation
        total_minutes = sum(w['avg_duration_minutes'] * w['run_count'] 
                           for w in self.analysis_results['workflow_performance'])
        ...
        if slow_workflows > 0 or bottleneck_jobs > 0:
            ...
            report_lines.extend([
                f"- **Potential Monthly Savings**: ${potential_savings:.2f} (with optimizations)",
                ""
            ])
