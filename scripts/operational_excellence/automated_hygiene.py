...
class RepositoryHygieneScanner:
        ...
        for line in branches_output.split('\n'):
            ...
            try:
                last_commit = self.run_git_command([
                    "log", "-1", "--format=%ci", f"origin/{branch_name}"
                ])
                if last_commit:
    ...
    def generate_report(self) -> str:
        ...
        # Executive Summary
        total_issues = (len(self.findings['stale_branches']) + 
        ...
        if self.findings['merged_branches'] or self.findings['stale_branches']:
            ...
            if self.findings['merged_branches']:
                ...
                report_lines.extend([
                    "",
                    "**Action Required**: Review and delete merged branches",
                    ""
                ])
            ...
            if self.findings['stale_branches']:
                ...
                report_lines.extend([
                    "",
                    "**Action Required**: Review stale branches for deletion or reactivation",
                    ""
                ])
        ...
        if self.findings['dead_links']:
            ...
            for link in self.findings['dead_links'][:10]:
                report_lines.append(f"- `{link['file']}:{link['line']}` - [{link['text']}]({link['url']}) ({link['type']})")
        ...
        if self.findings['new_todos']:
            report_lines.extend([
                f"## 📝 TODO Comments ({len(self.findings['new_todos'])})",
                "Technical debt items found in code:",
                ""
            ])
            ...
            report_lines.extend([
                "**Action Required**: Review TODO comments and create issues for important items",
                ""
            ])
        ...
        if self.findings['markdown_issues']:
            ...
            report_lines.extend([
                "",
                "**Action Required**: Fix documentation formatting issues",
                ""
            ])
        ...
        if total_issues == 0:
            report_lines.extend([
                "## ✅ All Clear!",
                "",
                "No hygiene issues found. Repository is in excellent condition!",
                ""
            ])
...
def main():
    ...
    if not any([args.analyze_branches, args.check_links, args.scan_todos, args.lint_docs, args.generate_report, args.all]):
        parser.print_help()
        return
