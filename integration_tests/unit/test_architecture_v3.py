    ...
    if study_service.exists():
        with open(study_service) as f:
            ...
            if 'from ecosystemiser.services.simulation_service' in content and '# Import only types' not in content:
                violations.append("study_service.py: Import from simulation_service should have comment explaining it's for types only")
...
def test_climate_validation_colocated():
    ...
    expected_profiles = {'nasa_power.py': 'NASAPowerQCProfile', 'meteostat.py': 'MeteostatQCProfile', 'era5.py': 'ERA5QCProfile'}
...
def test_cli_layer_purity():
    ...
    with open(cli_file) as f:
        content = f.read()
    forbidden_patterns = ['SystemRepository(', 'ConfigBuilder(', 'SystemBuilder(', 'SimulationEngine(', 'EnergySystemSimulator(']
    for pattern in forbidden_patterns:
        if pattern in content:
            violations.append(f'cli.py: Direct instantiation of {pattern} (should use service layer)')
    forbidden_imports = ['from ecosystemiser.repositories', 'from ecosystemiser.builders', 'from ecosystemiser.simulation.engine']
    ...
    required_imports = ['from ecosystemiser.services.simulation_service', 'from ecosystemiser.services.reporting_service']
...
def test_streamlit_isolation():
    ...
    for py_file in dashboard_dir.rglob('*.py'):
        forbidden_patterns = ['ecosystemiser.']
...
def test_validation_file_size():
    ...
    assert line_count < 1800, f'validation.py has {line_count} lines (should be < 1800 after refactoring). QCProfile classes should be moved to adapter files.'
