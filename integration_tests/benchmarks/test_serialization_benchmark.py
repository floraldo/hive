...
class SerializationBenchmark:
    ...
    def __init__(self):
        self.test_data = {'tiny': {'status': 'ok'}, 'small': {'key': 'value', 'number': 42, 'bool': True}, 'medium': {'data': 'x' * 1000, 'items': list(range(100)), 'nested': {'key': 'value', 'array': [1, 2, 3]}}, 'large': {'payload': 'x' * 10000, 'array': list(range(1000)), 'complex': {'users': [{'id': i, 'name': f'user{i}', 'active': i % 2 == 0} for i in range(100)]}}}
    ...
    def benchmark_cache_simulation(self) -> list[BenchmarkResult]:
        ...
        for size_name, data in self.test_data.items():
            ...
            def simulate_msgpack_cache_get():
                ...
                time.sleep(0.0001)
                return msgpack.unpackb(serialized, raw=False)
            for operation in [simulate_json_cache_set, simulate_msgpack_cache_set, simulate_json_cache_get, simulate_msgpack_cache_get]:
    ...
    def print_results(self, all_results: dict[str, list[BenchmarkResult]]):
        ...
        for category, results in all_results.items():
            ...
            for size, size_results in size_groups.items():
                ...
                for result in size_results:
                    status = 'FAST' if result.p95_ms < 0.1 else 'OK' if result.p95_ms < 1.0 else 'SLOW' if result.p95_ms < 5.0 else 'CRITICAL'
                    if result.p95_ms > 1.0:
                        critical_issues.append(f'{result.name}: {result.p95_ms:.3f}ms P95')
                    print(f'    {result.name:<46} {result.mean_ms:>6.3f}ms {result.p95_ms:>6.3f}ms {result.p99_ms:>6.3f}ms {result.ops_per_second:>10,.0f} {status}')
        print('\n' + '=' * 90)
        ...
        if all_results_flat:
            ...
            if serialize_ops:
                ...
                print(f'  Fastest: {fastest_serialize.name} ({fastest_serialize.p95_ms:.3f}ms P95, {fastest_serialize.ops_per_second:,.0f} ops/sec)')
                print(f'  Slowest: {slowest_serialize.name} ({slowest_serialize.p95_ms:.3f}ms P95, {slowest_serialize.ops_per_second:,.0f} ops/sec)')
            ...
            if deserialize_ops:
                ...
                print(f'  Fastest: {fastest_deserialize.name} ({fastest_deserialize.p95_ms:.3f}ms P95, {fastest_deserialize.ops_per_second:,.0f} ops/sec)')
                print(f'  Slowest: {slowest_deserialize.name} ({slowest_deserialize.p95_ms:.3f}ms P95, {slowest_deserialize.ops_per_second:,.0f} ops/sec)')
