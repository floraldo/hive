...
class AIPlannerIntegrationTests:
    ...
    def _create_execution_plan(self, planning_task_id: int, plan_data: dict[str, Any]) -> int:
        ...
        conn = sqlite3.connect(self.env.test_db_path)
        cursor = conn.execute("INSERT INTO execution_plans (planning_task_id, plan_data, status)\n               VALUES (?, ?, 'ready')", (planning_task_id, json.dumps(plan_data)))
    ...
    def test_5x_performance_improvement_validation(self) -> bool:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                futures = [executor.submit(concurrent_task_worker, [0, 1, 2, 3, 4]), executor.submit(concurrent_task_worker, [5, 6, 7, 8, 9])]
