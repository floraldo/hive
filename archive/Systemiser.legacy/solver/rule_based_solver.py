...
class ComponentBasedRuleEngine:
    ...
    def solve_timestep(self, t: int):
        ...
        for flow_key in sorted_flow_keys:
            ...
            if flow_amount > 1e-6:
                ...
                if (
                    "value" not in flow_data
                    or not isinstance(flow_data["value"], np.ndarray)
                    or len(flow_data["value"]) != self.system.N
                ):
                ...
                if from_comp.type == "storage":
                    ...
                    system_logger.debug(
                        f"   {from_name} (Storage State) level reduced by {energy_drawn:.3f} (flow {flow_amount:.3f}, eta {eta_discharge:.3f}) -> {state.components[from_name]['current_level']:.3f}"
                    )
                ...
                if to_comp.type == "storage":
                    ...
                    system_logger.debug(
                        f"   {to_name} (Storage State) level increased by {energy_stored:.3f} (flow {flow_amount:.3f}, eta {eta_charge:.3f}) -> {state.components[to_name]['current_level']:.3f}"
                    )
                # -----------------------------------------------------------------------------------
            else:
        ...
        for comp_name, comp in self.system.components.items():
            if comp.type == "storage":
                ...
                if hasattr(comp, "E") and isinstance(comp.E, np.ndarray) and t < len(comp.E):
                    ...
                    system_logger.error(
                        f"Cannot update E[{t}] for component {comp_name}. Array missing or index out of bounds."
                    )
                ...
                system_logger.debug(
                    f"    Final Level E[{t}]: {comp.E[t] if hasattr(comp, 'E') and t < len(comp.E) else 'Error'}"
                )
        ...
        if t < self.system.N - 1:
            for comp_name, comp in self.system.components.items():
                if comp.type == "storage":
                    if (
                        hasattr(comp, "E")
                        and isinstance(comp.E, np.ndarray)
                        and t < len(comp.E)
                        and not np.isnan(comp.E[t])
                    ):
                        ...
                        if t + 1 < len(comp.E):
                            ...
                            system_logger.error(
                                f"Index t+1 ({t+1}) out of bounds for {comp_name}.E array (len {len(comp.E)}). Cannot propagate."
                            )
                    else:
                        system_logger.error(
                            f"Storage level for {comp_name} is NaN or invalid at t={t}. Cannot propagate reliably."
                        )
                        # Fallback propagation
                        fallback_level = (
                            comp.E[t - 1]
                            if t > 0 and hasattr(comp, "E") and t < len(comp.E) and not np.isnan(comp.E[t - 1])
                            else comp.E_init
                        )
                        if t + 1 < len(comp.E):
                            ...
                            system_logger.error(
                                f"Index t+1 ({t+1}) out of bounds for {comp_name}.E array during fallback propagation."
                            )
    ...
    def _get_system_state(self, t) -> SystemState:
        ...
        for name, comp in self.system.components.items():
            ...
            if comp.type == "storage":
                ...
                if t == 0:
                    ...
                    system_logger.warning(
                        f"Could not determine storage level for {name} at start of t={t} from E[{t-1}], using E_init."
                    )
                else:
            ...
            if comp.type == "generation":
                ...
                power_limit = getattr(
                    comp, "P_max", getattr(comp, "W_cha_max", float("inf"))
                )  # Use W_cha_max for water components
            ...
            system_logger.debug(
                f"  {name} State Potentials: Avail={state_data['available_output']:.3f}, Req={state_data['required_input']:.3f}, EffC={eta_charge:.2f}, EffD={eta_discharge:.2f}"
            )
    ...
    def _initialize_arrays(self):
        ...
        for flow_key, flow_data in self.system.flows.items():
            ...
            if not (
                "value" in flow_data
                and isinstance(flow_data["value"], np.ndarray)
                and len(flow_data["value"]) == self.system.N
            ):
                flow_data["value"] = np.zeros(self.system.N)
            else:
    ...
    def _validate_balance(self, state: SystemState):
        ...
        for medium, components in media_components.items():
            ...
            for comp in components:
                ...
                if comp.type == "generation":
                    generation += sum(
                        self._get_value(f["value"], t)
                        for f in comp.flows.get("source", {}).values()
                        if f["type"] == medium
                    )
                elif comp.type == "consumption":
                    ...
                    consumption += sum(
                        self._get_value(f["value"], t)
                        for f in comp.flows.get("input", {}).values()
                        if f["type"] == medium
                    )
                    ...
                    discharge_flow = sum(
                        ...
                        for f_name, f in comp.flows.get("source", {}).items()
                        if f["type"] == medium
                    )  # Sum all source flows for this medium
                    ...
                    charge_flow = sum(
                        ...
                        for f_name, f in comp.flows.get("sink", {}).items()
                        if f["type"] == medium
                    )  # Sum all sink flows for this medium
