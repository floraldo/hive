...
class GenericRuleEngine:
    ...
    def solve_timestep(self, t: int):
        ...
        for name, comp in self.sys.components.items():
            if comp.type == "storage":
                ...
                in_flow = sum(self.sys.flows[key]["value"][t] for key in self.sys.flows if key.endswith(f"_to_{name}"))

                # Find total energy flowing *out of* the storage this timestep
                out_flow = sum(
                    self.sys.flows[key]["value"][t] for key in self.sys.flows if key.startswith(f"{name}_to_")
                )
                ...
                comp.E[t] = max(0, min(initial_level + in_flow - out_flow, getattr(comp, "E_max", float("inf"))))
                logger.debug(
                    f"  Storage {name}: E_init={initial_level:.2f}, in={in_flow:.2f}, out={out_flow:.2f}, E_final={comp.E[t]:.2f}"
                )
    ...
    def _get_state(self, t: int) -> SystemState:
        ...
        for name, comp in self.sys.components.items():
            ...
            logger.debug(
                f"  State t={t} {name}: src={comps_state[name]['source']:.2f}, sink={comps_state[name]['sink']:.2f}"
            )
    ...
    def _init_arrays(self):
        ...
        for key, flow_data in self.sys.flows.items():
            ...
            if not (
                hasattr(flow_data, "value")
                and isinstance(flow_data["value"], np.ndarray)
                and len(flow_data["value"]) == self.sys.N
            ):
        ...
        for name, comp in self.sys.components.items():
            if comp.type == "storage":
                ...
                if not (hasattr(comp, "E") and isinstance(comp.E, np.ndarray) and len(comp.E) == self.sys.N):
