...
class StandardWeatherProcessor(BaseWeatherProcessor):
    ...
    def __init__(self):
        ...
        self._bounded_params = {"relative_humidity_percent": 100, "precipitation_mm_hr": 50}  # Extreme rainfall cap
        ...
        for col, (min_val, max_val) in self._validation_bounds.items():
            ...
            if min_val is not None:
                ...
                if below_min_count > 0:
                    self.logger.warning(
                        f"Found {below_min_count} values below minimum ({min_val}) " f"for {col}. Setting to {min_val}"
                    )
            ...
            if max_val is not None:
                ...
                if above_max_count > 0:
                    self.logger.warning(
                        f"Found {above_max_count} values above maximum ({max_val}) " f"for {col}. Capping at {max_val}"
                    )
