...
class SystemiserVisualizer:
    ...
    def plot_energy_flows(self) -> plt.Figure:
        ...
        for flow_name in major_flows['Flow_Name'].unique():
            ...
            ax1.plot(flow_data['Hour'], flow_data['Value'], 
                    marker='o', linewidth=2, label=flow_name, markersize=4)
        ...
        if not storage_levels.empty:
            for component in storage_levels['Component'].unique():
                ...
                ax3.plot(comp_data['Hour'], comp_data['Storage_Level'], 
                        marker='s', linewidth=3, label=component, markersize=4)
    ...
    def plot_water_flows(self) -> plt.Figure:
        ...
        ax4.bar(net_flows.index, net_flows.values, alpha=0.7, 
               color=['green' if x >= 0 else 'red' for x in net_flows.values])
    ...
    def create_summary_dashboard(self) -> plt.Figure:
        ...
        fig.suptitle('🌟 Systemiser Simulation Dashboard - 24 Hour Analysis', 
        ...
        if not energy_storage.empty:
            ...
            if not battery.empty:
                for comp in battery['Component'].unique():
                    ...
                    ax3.plot(comp_data['Hour'], comp_data['Storage_Level'], 
                           linewidth=3, marker='o', markersize=3, label=comp)
        ...
        if not water_storage.empty:
            for comp in water_storage['Component'].unique():
                ...
                ax7.plot(comp_data['Hour'], comp_data['Storage_Level'], 
                       linewidth=3, marker='s', markersize=3, label=comp)
        ...
        if not energy_flows.empty:
            total_solar = energy_flows[energy_flows['Source'].str.contains('Solar|PV', case=False, na=False)]['Value'].sum()
            total_consumption = energy_flows[energy_flows['Target'].str.contains('Demand|Load', case=False, na=False)]['Value'].sum()
        ...
        if not water_flows.empty:
            total_rainwater = water_flows[water_flows['Source'].str.contains('Rain', case=False, na=False)]['Value'].sum()
            total_water_demand = water_flows[water_flows['Target'].str.contains('Demand|Use', case=False, na=False)]['Value'].sum()
            summary_text += f"\n💧 Water System:\n"
        ...
        ax10.text(0.05, 0.95, quality_text, transform=ax10.transAxes, fontsize=12,
                 verticalalignment='top', fontfamily='monospace',
