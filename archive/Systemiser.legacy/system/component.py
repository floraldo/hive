...
class Component:
    ...
    def get_economic_results(self):
        ...
        self.economic.opex_lifetime = self.economic.get_opex(
            self.P_max, self.energy_throughput, self.N, years=self.economic.lifetime
        )
        self.economic.revenue_per_year = self.economic.get_revenue(self.energy_generated, self.N, years=1)
        self.economic.revenue_lifetime = self.economic.get_revenue(
            self.energy_generated, self.N, years=self.economic.lifetime
        )
        ...
        economical_results_df = pd.DataFrame(list(self.economical_results.items()), columns=["Metric", "Value"]).round(
            2
        )
    ...
    def get_environmental_results(self):
        ...
        self.environmental.co2_emissions_lifetime = self.environmental.calculate_emissions_over_time(
            relevant_size, total_energy, lifetime, years=lifetime, distribute_embedded=True, N=self.N
        )["Yearly CO2 Emissions (kg)"].sum()
        self.environmental.co2_emissions_30years = self.environmental.calculate_emissions_over_time(
            relevant_size, total_energy, lifetime, years=30, distribute_embedded=True, N=self.N
        )["Yearly CO2 Emissions (kg)"].sum()
        ...
        environmental_results_df = pd.DataFrame(
            list(self.environmental_results.items()), columns=["Metric", "Value"]
        ).round(2)
    ...
    def set_constraints(self):
        for t in range(self.N):
            input_flows = cp.sum(
                [
                    -flow["value"][t]
                    for flow_name, flow in self.flows["input"].items()
                    if isinstance(flow["value"], cp.Variable)
                ]
            )
            output_flows = cp.sum(
                [
                    flow["value"][t]
                    for flow_name, flow in self.flows["output"].items()
                    if isinstance(flow["value"], cp.Variable)
                ]
            )
            source_flows = cp.sum(
                [
                    flow["value"][t]
                    for flow_name, flow in self.flows["source"].items()
                    if isinstance(flow["value"], cp.Variable)
                ]
            )
            sink_flows = cp.sum(
                [
                    -flow["value"][t]
                    for flow_name, flow in self.flows["sink"].items()
                    if isinstance(flow["value"], cp.Variable)
                ]
            )
            # ENERGY BALANCE
