...
def solar_radiation_to_pv_profile(
    solar_radiation_wm2: pd.Series, pv_capacity_kw: float, efficiency: float = 0.20, system_losses: float = 0.15
) -> pd.Series:
    """
    ...
    logger.info(
        f"Generated PV profile with max output: {pv_output.max():.2f} kW, " f"average: {pv_output.mean():.2f} kW"
    )

    return pv_output


def temperature_to_heat_demand_profile(
    temperature_celsius: pd.Series,
    base_demand_kw: float,
    heating_threshold_c: float = 15.0,
    design_temp_c: float = -10.0,
    design_demand_multiplier: float = 3.0,
) -> pd.Series:
    """
    ...
    logger.info(
        f"Generated heat demand profile with max demand: {heat_demand.max():.2f} kW, "
...
def wind_speed_to_wind_profile(
    ...
    wind_output[ramp_mask] = (
        turbine_capacity_kw * ((wind_speed_ms[ramp_mask] - cut_in_speed) / (rated_speed - cut_in_speed)) ** 3
    )
    ...
    logger.info(
        f"Generated wind profile with max output: {wind_output.max():.2f} kW, " f"average: {wind_output.mean():.2f} kW"
    )

    return wind_output


def precipitation_to_rainwater_profile(
    precipitation_mm_hr: pd.Series, collection_area_m2: float, collection_efficiency: float = 0.8
) -> pd.Series:
    """
    ...
    logger.info(
        f"Generated rainwater profile with max collection: {rainwater_collection.max():.2f} L/hr, "
...
def create_component_profiles_from_weather(
    weather_df: pd.DataFrame, component_configs: Dict[str, Dict[str, Any]]
) -> Dict[str, pd.Series]:
    """
...
def calculate_weather_statistics(weather_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    stats["data_quality"] = {
        ...
        "date_range": {"start": weather_df.index.min(), "end": weather_df.index.max()},
...
if __name__ == "__main__":
    ...
    component_configs = {
        "pv_system": {"type": "solar_pv", "capacity_kw": 100, "efficiency": 0.20},
        "heat_demand": {"type": "heat_demand", "base_demand_kw": 50},
        "wind_turbine": {"type": "wind_turbine", "capacity_kw": 25},
        "rainwater": {"type": "rainwater_collection", "collection_area_m2": 200},
    }
