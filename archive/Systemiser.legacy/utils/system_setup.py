        ...
        if system_type == "energy":
            components = {
                ...
                "SOLAR_THERMAL": SolarThermal(
                    "SOLAR_THERMAL", P_profile=profiles["solar_thermal"]["data"], P_max=20, n=N
                ),
                "BASE_LOAD": PowerDemand("BASE_LOAD", P_profile=profiles["base_load"]["data"], P_max=15, n=N),
                "SPACE_HEATING": HeatDemand(
                    "SPACE_HEATING", P_profile=profiles["space_heating"]["data"], P_max=15, n=N
                ),
            ...
            components = {
                "WATER_POND": WaterPond("WATER_POND", W_cha_max=20, E_max=200, E_init=100, infiltration_rate=0.1, n=N),
                ...
                "DRINKING_WATER": WaterDemand(
                    "DRINKING_WATER", W_profile=profiles["water_demand"]["data"], W_max=8, n=N
                ),
                "EVAPORATION": EvaporationDemand(
                    "EVAPORATION", temp_profile=profiles["temperature"]["data"], W_max=3000 * 10 / (1000 * 24), n=N
                ),  # Max rate needs clarification
...
def create_objective(system, system_type):
    ...
    try:
        if system_type == "energy":
            ...
            water_grid_comp = next(
                (comp for comp in system.components.values() if comp.type == "transmission" and comp.medium == "water"),
                None,
            )
            overflow_comp = next(
                (comp for comp in system.components.values() if comp.name == "OVERFLOW"), None
            )  # Assuming name
