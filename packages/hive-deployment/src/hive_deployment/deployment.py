...
def connect_to_server(config: Dict[str, Any]) -> Optional[SSHClient]:
    """
...
def deploy_setup_venv(ssh: SSHClient, venv_path: str, req_path: str, config: Dict[str, Any]) -> bool:
    ...
    exit_code, stdout, stderr = run_remote_command(ssh, f"'{pip_path}' install -r '{req_path}'", config=config, check=False)
    if exit_code != 0:
...
def deploy_update_env_file(ssh: SSHClient, env_file_path: str, port: int, config: Dict[str, Any]) -> bool:
    ...
    exit_code, _, stderr = run_remote_command(ssh, f"echo 'FLASK_RUN_PORT={port}' >> {env_file_path}", config=config, check=False)
    if exit_code != 0:
...
def deploy_set_permissions(ssh: SSHClient, remote_app_dir: str, config: Dict[str, Any], deployment_config: Optional[Dict[str, str]] = None) -> bool:
    ...
    # Set permissions
    exit_code, _, stderr = run_remote_command(ssh, f"chmod -R u=rwx,g=rwx,o=rx '{remote_app_dir}'", config=config, sudo=True, check=False)
    if exit_code != 0:
        log.error(f"Failed to set base permissions (775): {stderr}")
        return False
    
    # Ensure instance/static directories exist
    run_remote_command(ssh, f"mkdir -p '{remote_app_dir}/instance' '{remote_app_dir}/static'", config=config, sudo=True, check=False)
    
    # Set group ID bit and write permissions
    run_remote_command(ssh, f"chmod g+s '{remote_app_dir}/instance' '{remote_app_dir}/static'", config=config, sudo=True, check=False)
...
WantedBy=multi-user.target"""
    ...
    exit_code, _, stderr = run_remote_command(ssh, f"mv -f {temp_service_path} {systemd_service_path}", config=config, sudo=True, check=False)
    if exit_code != 0:
    ...
    exit_code, _, stderr = run_remote_command(ssh, f"systemctl restart {app_name}.service", config=config, sudo=True, check=False)
    if exit_code != 0:
        log.error(f"Failed to start/restart {app_name}.service: {stderr}")
        # Show service status and logs for debugging
        run_remote_command(ssh, f"systemctl status {app_name}.service --no-pager -l", config=config, sudo=True, check=False)
        run_remote_command(ssh, f"journalctl -u {app_name}.service --no-pager -n 50", config=config, sudo=True, check=False)
        return False
...
"""
    ...
    exit_code, _, stderr = run_remote_command(ssh, f"mv -f {temp_nginx_path} {nginx_conf_path}", config=config, sudo=True, check=False)
    if exit_code != 0:
...
def execute_deployment_steps(
    ...
    try:
        ...
        if not deploy_systemd_service(
            ssh, app_name, paths['remote_app_dir'], paths['venv_path'],
            paths['systemd_service_path'], assigned_port, config
        ):
