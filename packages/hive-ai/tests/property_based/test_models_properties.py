...
async def test_metrics_aggregation_properties(costs):
    ...
    # Property: Request count should equal number of recordings
...
def test_registry_model_count_properties(model_names):
    ...
    # Property: Number of available models should equal number added
    available = registry.list_available_models()
    assert len(available) >= len(model_names)  # May include defaults

    # Property: All added models should be retrievable
    for name in model_names:
...
async def test_success_rate_properties(success_count, failure_count):
    ...
    assume(success_count + failure_count > 0)  # At least one operation
    ...
    # Property: Successful requests should match count
    assert summary["successful_requests"] == success_count

    # Property: Total requests should match sum
