...
def prompt_template_data(draw):
    ...
    variable_names = draw(st.lists(st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'), whitelist_characters='_')), min_size=0, max_size=5, unique=True))
...
def agent_config_data(draw):
    ...
    return {'name': draw(st.text(min_size=1, max_size=50)), 'description': draw(st.text(min_size=1, max_size=200)), 'model': draw(st.sampled_from(['claude-3-sonnet', 'gpt-4', 'test-model'])), 'temperature': draw(st.floats(min_value=0.0, max_value=2.0)), 'max_tokens': draw(st.integers(min_value=100, max_value=8192)), 'max_iterations': draw(st.integers(min_value=1, max_value=50))}
...
class TestPromptTemplate:
    ...
    def test_template_creation_simple(self):
        ...
        template = PromptTemplate(template='Hello {{ name }}!', variables=[PromptVariable(name='name', type='str', required=True)])
    ...
    def test_template_render_success(self):
        ...
        template = PromptTemplate(template='Hello {{ name }}, you are {{ age }} years old.', variables=[PromptVariable(name='name', type='str', required=True), PromptVariable(name='age', type='int', required=True)])
    ...
    def test_template_variable_validation(self):
        ...
        template = PromptTemplate(template='Number: {{ num }}', variables=[PromptVariable(name='num', type='int', required=True)])
    ...
    def test_template_default_values(self):
        ...
        template = PromptTemplate(template='Hello {{ name }}, welcome to {{ place }}!', variables=[PromptVariable(name='name', type='str', required=True), PromptVariable(name='place', type='str', required=False, default='our site')])
    ...
    def test_template_cloning(self):
        ...
        original = PromptTemplate(template='Original {{ var }}', variables=[PromptVariable(name='var', type='str')], metadata=PromptMetadata(name='original', description='test'))
    ...
    def test_template_serialization(self):
        ...
        template = PromptTemplate(template='Test {{ var }}', variables=[PromptVariable(name='var', type='str', description='test var')], metadata=PromptMetadata(name='test', description='test template'))
    ...
    def test_template_property_based(self, template_data):
        ...
        variables = [PromptVariable(name=var_name, type='str', required=True, default='default') for var_name in template_data['variables']]
        try:
...
class TestPromptOptimizer:
    ...
    def mock_model_client(self):
        ...
        client = Mock()
        client.generate_async = AsyncMock(return_value=Mock(content='Optimized prompt: The improved version\n\nImprovements:\n1. Better clarity\n2. More specific\n\nOptimized prompt:'))
        return client
...
class TestPromptRegistry:
    ...
    async def test_register_template_async(self, registry):
        ...
        template = PromptTemplate(template='Test {{ var }}', metadata=PromptMetadata(name='test_template', description='A test template'))
    ...
    async def test_clone_template_async(self, registry):
        """Test template cloning."""
        original = PromptTemplate(template='Original {{ var }}', metadata=PromptMetadata(name='original', description='original template'))
        await registry.register_template_async(original)
        cloned_name = await registry.clone_template_async('original', 'cloned', modifications={'template': 'Cloned {{ var }}'})
    ...
    async def test_export_import_templates_async(self, registry, temp_registry_dir):
        ...
        template = PromptTemplate(template='Export test {{ var }}', metadata=PromptMetadata(name='export_test', description='for export'))
...
class TestSimpleTaskAgent:
    ...
    def mock_model_client(self):
        """Mock ModelClient for testing."""
        client = Mock()
        client.generate_async = AsyncMock(return_value=Mock(content='Task completed successfully', tokens_used=25, cost=0.0005))
        return client
...
class TestWorkflowOrchestrator:
    ...
    def workflow_config(self):
        ...
        return WorkflowConfig(name='test_workflow', description='A test workflow', execution_strategy=ExecutionStrategy.SEQUENTIAL, max_concurrent_steps=2)

    @pytest.fixture
    def mock_model_client(self):
        """Mock ModelClient for testing."""
        client = Mock()
        client.generate_async = AsyncMock(return_value=Mock(content='Workflow step completed', tokens_used=30, cost=0.0006))
        return client
    ...
    def test_workflow_step_creation(self, workflow_config, mock_model_client):
        ...
        task_id = workflow.add_task(task)
        step_id = workflow.create_step(name='test_step', agent_id=agent_id, task_id=task_id, dependencies=[], timeout_seconds=60)
...
class TestIntegrationScenarios:
    ...
    async def test_prompt_agent_integration_async(self):
        ...
        template = PromptTemplate(template='Analyze this data: {{ data }}\nProvide insights about {{ focus_area }}', variables=[PromptVariable(name='data', type='str', required=True), PromptVariable(name='focus_area', type='str', required=True)])
        agent_config = AgentConfig(name='analysis_agent', description='Data analysis agent', model='test-model')
        mock_client = Mock()
        mock_client.generate_async = AsyncMock(return_value=Mock(content='Analysis complete: The data shows clear trends in the focus area.', tokens_used=45, cost=0.0009))
    ...
    async def test_multi_step_workflow_property_async(self, step_descriptions):
        ...
        config = WorkflowConfig(name='property_test_workflow', description='Property-based workflow test', execution_strategy=ExecutionStrategy.SEQUENTIAL)
        mock_client = Mock()
        ...
        try:
            for i, description in enumerate(step_descriptions):
                ...
                workflow.create_step(name=f'step_{i}', agent_id=agent_id, task_id=task_id, dependencies=dependencies)
            await workflow.initialize_async()
            assert workflow.status == WorkflowStatus.INITIALIZED
