...
class TestTaskResult:
    ...
    def test_task_result_creation_minimal(self):
        ...
        result = TaskResult(task_id='task-123', status=TaskStatus.COMPLETED, result={'output': 'success'})
    ...
    def test_task_result_creation_complete(self):
        ...
        result = TaskResult(task_id='task-456', status=TaskStatus.FAILED, result=None, error='Execution failed', start_time=start_time, end_time=end_time, duration_seconds=3.5, metadata=metadata)
...
class TestTaskDependency:
    ...
    def test_task_dependency_with_condition(self):
        ...
        dependency = TaskDependency(task_id='task-2', dependency_type='output', condition=custom_condition)
...
class TestTaskConfig:
    ...
    def test_task_config_creation_minimal(self):
        ...
        config = TaskConfig(name='test_task', description='Test task description')
    ...
    def test_task_config_creation_complete(self):
        ...
        config = TaskConfig(name='complex_task', description='Complex task', priority=TaskPriority.HIGH, timeout_seconds=600, retry_attempts=5, retry_delay_seconds=10, dependencies=dependencies, required_tools=required_tools, metadata=metadata)
...
class TestPromptTask:
    ...
    def mock_agent(self):
        ...
        agent.model_client.generate_async = AsyncMock(return_value=Mock(content='Test response', tokens_used=50, cost=0.001, model='claude-3-sonnet'))
        return agent

    @pytest.fixture
    def basic_task_config(self):
        """Create a basic TaskConfig."""
        return TaskConfig(name='prompt_task', description='Test prompt task')

    @pytest.mark.core
    def test_prompt_task_creation(self, basic_task_config):
        """Test creating PromptTask."""
        task = PromptTask(config=basic_task_config, prompt='Analyze this data')
    ...
    def test_prompt_task_creation_with_output_type(self, basic_task_config):
        ...
        task = PromptTask(config=basic_task_config, prompt='Calculate sum', expected_output_type='number')
    ...
    async def test_prompt_task_execute_basic(self, basic_task_config, mock_agent):
        ...
        task = PromptTask(config=basic_task_config, prompt='Test prompt')
    ...
    async def test_prompt_task_execute_with_input_data(self, basic_task_config, mock_agent):
        ...
        task = PromptTask(config=basic_task_config, prompt='Process data')
        task.start_time = datetime.utcnow()
    ...
    async def test_prompt_task_execute_with_dependencies(self, basic_task_config, mock_agent):
        ...
        task = PromptTask(config=basic_task_config, prompt='Summarize results')
        task.start_time = datetime.utcnow()
        dependency_results = {'dep-1': TaskResult(task_id='dep-1', status=TaskStatus.COMPLETED, result='Previous result')}
    ...
    async def test_prompt_task_process_json_output(self, basic_task_config, mock_agent):
        ...
        mock_agent.model_client.generate_async = AsyncMock(return_value=Mock(content='{"key": "value"}', tokens_used=30, cost=0.0005, model='claude-3-sonnet'))
        task = PromptTask(config=basic_task_config, prompt='Generate JSON', expected_output_type='json')
        task.start_time = datetime.utcnow()
        result = await task.execute_async(mock_agent)
        assert result.status == TaskStatus.COMPLETED
    ...
    async def test_prompt_task_process_number_output(self, basic_task_config, mock_agent):
        ...
        mock_agent.model_client.generate_async = AsyncMock(return_value=Mock(content='42.5', tokens_used=10, cost=0.0001, model='claude-3-sonnet'))
        task = PromptTask(config=basic_task_config, prompt='Calculate value', expected_output_type='number')
        task.start_time = datetime.utcnow()
        result = await task.execute_async(mock_agent)
        assert result.status == TaskStatus.COMPLETED
    ...
    async def test_prompt_task_process_boolean_output(self, basic_task_config, mock_agent):
        ...
        mock_agent.model_client.generate_async = AsyncMock(return_value=Mock(content='true', tokens_used=5, cost=5e-05, model='claude-3-sonnet'))
        task = PromptTask(config=basic_task_config, prompt='Is valid?', expected_output_type='boolean')
        task.start_time = datetime.utcnow()
        result = await task.execute_async(mock_agent)
        assert result.status == TaskStatus.COMPLETED
...
class TestToolTask:
    ...
    def basic_task_config(self):
        """Create a basic TaskConfig."""
        return TaskConfig(name='tool_task', description='Test tool task')
    ...
    def test_tool_task_creation(self, basic_task_config):
        ...
        task = ToolTask(config=basic_task_config, tool_sequence=tool_sequence)
    ...
    async def test_tool_task_execute_multiple_tools(self, basic_task_config, mock_agent):
        ...
        tool_sequence = [{'tool': 'add', 'parameters': {'x': 5, 'y': 3}}, {'tool': 'multiply', 'parameters': {'x': 2, 'y': 4}}]
        task = ToolTask(config=basic_task_config, tool_sequence=tool_sequence)
        task.start_time = datetime.utcnow()
        result = await task.execute_async(mock_agent)
        assert result.status == TaskStatus.COMPLETED
...
class TestBaseTask:
    ...
    def basic_task_config(self):
        """Create a basic TaskConfig."""
        return TaskConfig(name='test_task', description='Test task')
    ...
    def mock_agent(self):
        ...
        agent = Mock(spec=SimpleTaskAgent)
        agent.model_client = Mock()
        agent.model_client.generate_async = AsyncMock(return_value=Mock(content='Result', tokens_used=50, cost=0.001, model='test-model'))
    ...
    def test_base_task_can_execute_dependency_met(self, basic_task_config):
        ...
        TaskConfig(name='dep_task', description='Dependency task')
        task_config = TaskConfig(name='main_task', description='Main task', dependencies=[TaskDependency(task_id='dep-1')])
        task = PromptTask(config=task_config, prompt='Test')
        completed_tasks = {'dep-1': TaskResult(task_id='dep-1', status=TaskStatus.COMPLETED, result='Dependency result')}
    ...
    def test_base_task_cannot_execute_dependency_failed(self, basic_task_config):
        ...
        task_config = TaskConfig(name='main_task', description='Main task', dependencies=[TaskDependency(task_id='dep-1')])
        task = PromptTask(config=task_config, prompt='Test')
        completed_tasks = {'dep-1': TaskResult(task_id='dep-1', status=TaskStatus.FAILED, result=None, error='Dependency failed')}
    ...
    def test_base_task_get_dependency_results(self, basic_task_config):
        ...
        task_config = TaskConfig(name='main_task', description='Main task', dependencies=[TaskDependency(task_id='dep-1'), TaskDependency(task_id='dep-2')])
        task = PromptTask(config=task_config, prompt='Test')
        completed_tasks = {'dep-1': TaskResult(task_id='dep-1', status=TaskStatus.COMPLETED, result='Result 1'), 'dep-2': TaskResult(task_id='dep-2', status=TaskStatus.COMPLETED, result='Result 2'), 'other': TaskResult(task_id='other', status=TaskStatus.COMPLETED, result='Other result')}
    ...
    async def test_base_task_execute_with_retry_success_after_failures(self, basic_task_config):
        ...
        config = TaskConfig(name='retry_task', description='Retry test', retry_attempts=3, retry_delay_seconds=0.01)
    ...
    async def test_base_task_execute_with_retry_all_attempts_fail(self, basic_task_config):
        ...
        mock_agent.config = Mock(model='test-model', temperature=0.7, max_tokens=4096)
        config = TaskConfig(name='fail_task', description='Fail test', retry_attempts=2, retry_delay_seconds=0.01)
        task = PromptTask(config=config, prompt='Test')
        result = await task._execute_with_retry_async(mock_agent)
        assert result.status == TaskStatus.FAILED
        assert task.attempt_count == 2
...
class TestTaskSequence:
    ...
    def test_task_sequence_creation(self):
        ...
        task1 = PromptTask(config=TaskConfig(name='task1', description='Task 1'), prompt='First task')
        task2 = PromptTask(config=TaskConfig(name='task2', description='Task 2'), prompt='Second task')
    ...
    def test_task_sequence_execution_order_no_dependencies(self):
        ...
        task1 = PromptTask(config=TaskConfig(name='task1', description='Task 1'), prompt='First')
        task2 = PromptTask(config=TaskConfig(name='task2', description='Task 2'), prompt='Second')
    ...
    def test_task_sequence_execution_order_with_dependencies(self):
        ...
        task1 = PromptTask(config=TaskConfig(name='task1', description='Task 1'), prompt='First')
        task1.id = task1_id
        task2 = PromptTask(config=TaskConfig(name='task2', description='Task 2', dependencies=[TaskDependency(task_id=task1_id)]), prompt='Second')
        sequence = TaskSequence(name='sequence', tasks=[task1, task2])
        assert len(sequence.execution_order) == 2
    ...
    def test_task_sequence_circular_dependency_detection(self):
        ...
        task1 = PromptTask(config=TaskConfig(name='task1', description='Task 1', dependencies=[TaskDependency(task_id='task-2')]), prompt='First')
        task1.id = 'task-1'
        task2 = PromptTask(config=TaskConfig(name='task2', description='Task 2', dependencies=[TaskDependency(task_id='task-1')]), prompt='Second')
        task2.id = 'task-2'
...
class TestTaskBuilder:
    ...
    def test_create_prompt_task_basic(self):
        ...
        task = TaskBuilder.create_prompt_task(name='test_task', prompt='Test prompt')
    ...
    def test_create_prompt_task_with_options(self):
        ...
        task = TaskBuilder.create_prompt_task(name='advanced_task', prompt='Advanced prompt', description='Custom description', priority=TaskPriority.HIGH, expected_output_type='json', dependencies=['dep-1', 'dep-2'])
    ...
    def test_create_tool_task_basic(self):
        ...
        task = TaskBuilder.create_tool_task(name='tool_task', tool_sequence=tool_sequence)
    ...
    def test_create_tool_task_extracts_required_tools(self):
        ...
        tool_sequence = [{'tool': 'tool1', 'parameters': {}}, {'tool': 'tool2', 'parameters': {}}, {'tool': 'tool1', 'parameters': {}}]
        task = TaskBuilder.create_tool_task(name='multi_tool_task', tool_sequence=tool_sequence)
    ...
    def test_create_analysis_task(self):
        ...
        task = TaskBuilder.create_analysis_task(name='analysis_task', analysis_prompt='Analyze the data', data_source='dataset.csv')
        assert isinstance(task, PromptTask)
