...
class TestAIWorkflowIntegration:
    ...
    async def test_model_vector_integration(self, ai_config, vector_config):
        ...
        texts = [
            ...
            "A third document to complete our test set.",
        ]
    ...
    async def test_prompt_agent_integration(self, ai_config):
        ...
        template = PromptTemplate(
            template="Analyze this data: {{ data }} and provide insights about {{ focus }}",
            variables=[
                PromptVariable(name="data", type="str", required=True),
                PromptVariable(name="focus", type="str", required=True),
            ],
            metadata=PromptMetadata(name="analysis_prompt", description="Data analysis template"),
        )
    ...
    async def test_end_to_end_ai_pipeline(self, ai_config):
        ...
        for step in pipeline_steps:
            # Each step would integrate multiple components
    ...
    async def test_error_handling_integration(self, ai_config):
        ...
        # Test that errors in one component are properly handled by others
        mock_client = Mock()
    ...
    async def test_performance_integration(self, ai_config):
        ...
        assert total_time < 1.0  # Should be much faster than 5 * 0.1 = 0.5 seconds
    ...
    async def test_configuration_integration(self, ai_config):
        ...
        # Components should reflect the configuration change
