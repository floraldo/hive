...
def test_security_performance():
    ...
    test_prompts = ["Simple test", "A" * 1000, "A" * 10000, "javascript:alert('test')"]
    validation_times = []
    ...
    results["input_validation"] = {"avg_ms": statistics.mean(validation_times), "max_ms": max(validation_times), "operations": len(validation_times)}
    ...
    results["secret_masking"] = {"total_ms": mask_duration, "avg_ms": mask_duration / len(secrets), "operations": len(secrets)}
...
def test_config_performance():
    ...
    start = time.perf_counter()
    for i in range(100):
        model_config = ModelConfig(name=f"test_model_{i}", provider="test", model_type="completion", max_tokens=4096, temperature=0.7, cost_per_token=0.001)
        model_configs.append(model_config)
    model_creation_duration = (time.perf_counter() - start) * 1000
    results["model_config_creation"] = {"total_ms": model_creation_duration, "avg_ms": model_creation_duration / 100, "operations": 100}
    start = time.perf_counter()
    for config in model_configs:
        config.model_dump()
    serialize_duration = (time.perf_counter() - start) * 1000
    results["config_serialization"] = {"total_ms": serialize_duration, "avg_ms": serialize_duration / len(model_configs), "operations": len(model_configs)}
    return results
...
def test_vector_performance():
    ...
    try:
        ...
        results["embedding_generation"] = {"avg_ms": statistics.mean(embedding_times) if embedding_times else 0, "max_ms": max(embedding_times) if embedding_times else 0, "operations": len(embedding_times)}
        try:
            vec1 = ([0.1] * 384,)
            vec2 = ([0.2] * 384,)
            start = (time.perf_counter(),)
...
def test_model_system_performance():
    ...
    try:
        ...
        test_models = []
        for i in range(20):
            model = ModelConfig(name=f"perf_test_model_{i}", provider="test", model_type="completion", max_tokens=4096, cost_per_token=0.001)
            test_models.append(model)
        start = time.perf_counter()
        for model in test_models:
            registry._register_model(model)
        registration_duration = (time.perf_counter() - start) * 1000
        results["model_registration"] = {"total_ms": registration_duration, "avg_ms": registration_duration / len(test_models), "operations": len(test_models)}
        start = time.perf_counter()
        for _ in range(100):
...
def run_performance_tests():
    ...
    test_suites = [("security", test_security_performance), ("config", test_config_performance), ("vector", test_vector_performance), ("model_system", test_model_system_performance)]
    ...
    with open(output_file, "w") as f:
        json.dump({"timestamp": time.time(), "total_duration_ms": total_duration, "summary": summary, "detailed_results": all_results}, f, indent=2)
    logger.info("\n=== PERFORMANCE SUMMARY ===")
...
def generate_summary(results: dict[str, Any], total_duration: float) -> dict[str, Any]:
        ...
        if "results" in suite_data:
            for test_name, test_data in suite_data["results"].items():
                ...
                if "avg_ms" in test_data:
                    operations.append({"name": f"{suite_name}.{test_name}", "avg_ms": test_data["avg_ms"], "operations": test_data.get("operations", 1)})
                elif "duration_ms" in test_data:
                    operations.append({"name": f"{suite_name}.{test_name}", "avg_ms": test_data["duration_ms"], "operations": 1})
    ...
    return {"total_operations": len(operations), "errors": errors, "fastest_operations": operations[:5], "slowest_operations": operations[-5:], "avg_operation_time": statistics.mean([op["avg_ms"] for op in operations]) if operations else 0, "recommendations": recommendations[:5]}
