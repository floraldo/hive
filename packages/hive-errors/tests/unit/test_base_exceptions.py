...
class TestBaseError:
    ...
    def test_base_error_full_parameters(self):
        ...
        error = BaseError(message='Test error', component='test_component', operation='test_operation', details={'key': 'value'}, recovery_suggestions=['Retry', 'Check config'], original_error=original)
        assert error.message == 'Test error'
    ...
    def test_base_error_to_dict(self):
        ...
        original = (ValueError('Original error'),)
        error = BaseError(message='Test error', component='test_component', operation='test_operation', details={'key': 'value'}, recovery_suggestions=['Retry'], original_error=original)
...
class TestCircuitBreakerOpenError:
    ...
    def test_circuit_breaker_error_custom_values(self):
        ...
        error = CircuitBreakerOpenError(message='Custom message', component='custom_component', operation='test_op', failure_count=5, recovery_time=30.0)
    ...
    def test_circuit_breaker_error_to_dict(self):
        ...
        error = CircuitBreakerOpenError(failure_count=5, recovery_time=30.0)
        error_dict = error.to_dict()
...
class TestAsyncTimeoutError:
    ...
    def test_async_timeout_error_custom_values(self):
        ...
        error = AsyncTimeoutError(message='Operation exceeded timeout', component='api', operation='fetch_data', timeout_duration=5.0, elapsed_time=7.5)
    ...
    def test_async_timeout_error_to_dict(self):
        ...
        error = AsyncTimeoutError(timeout_duration=5.0, elapsed_time=7.5)
        error_dict = error.to_dict()
...
class TestRetryExhaustedError:
    ...
    def test_retry_exhausted_error_custom_values(self):
        ...
        error = RetryExhaustedError(message='Retries exhausted after 3 attempts', component='api_client', operation='fetch', max_attempts=3, attempt_count=3, last_error=last_error)
    ...
    def test_retry_exhausted_error_to_dict(self):
        ...
        last_error = (ValueError('Connection failed'),)
        error = RetryExhaustedError(max_attempts=3, attempt_count=3, last_error=last_error)
        error_dict = error.to_dict()
...
class TestPoolExhaustedError:
    ...
    def test_pool_exhausted_error_custom_values(self):
        ...
        error = PoolExhaustedError(message='All connections in use', component='database_pool', operation='acquire', pool_size=10, active_connections=10)
    ...
    def test_pool_exhausted_error_to_dict(self):
        ...
        error = PoolExhaustedError(pool_size=10, active_connections=10)
        error_dict = error.to_dict()
...
class TestErrorInheritance:
    ...
    @pytest.mark.core
    @pytest.mark.parametrize('error_class', [ConfigurationError, ConnectionError, ValidationError, TimeoutError, ResourceError, CircuitBreakerOpenError, AsyncTimeoutError, RetryExhaustedError, PoolExhaustedError])
