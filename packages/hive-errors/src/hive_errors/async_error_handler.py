...
class AsyncErrorHandler:
    ...
    async def handle_success_async(self, context: ErrorContext, execution_time: float) -> None:
        ...
        await self._update_success_rates_async(context.operation_name, success=True)
    ...
    async def _update_error_stats_async(self, error: Exception, context: ErrorContext) -> None:
        ...
        recent_errors = [e for e in self._error_history if (datetime.utcnow() - e["timestamp"]).total_seconds() <= 60]
    ...
    async def _update_success_rates_async(self, operation: str, success: bool) -> None:
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_error_time": self._error_stats.last_error_time.isoformat()
    ...
    def get_recent_errors(self, component: str | None = None, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    async def predict_failure_risk_async(self, component: str, operation: str) -> Dict[str, Any]:
...
async def error_context_async(
    ...
    **context_kwargs
    ...
    try:
        ...
        await handler.handle_success_async(context, execution_time)
        ...
        processed_error = await handler.handle_error_async(timeout_error, context, suppress_errors)
        if not suppress_errors and processed_error:
            raise processed_error from e

    except Exception as e:
        processed_error = await handler.handle_error_async(e, context, suppress_errors)
        if not suppress_errors and processed_error:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper_async(*args, **kwargs) -> None:
            ...
            for attempt in range(max_retries + 1):
                ...
                try:
                    async with error_context_async(
                        handler
                        operation_name
                        component
                        timeout=timeout
                    ...
                    if attempt < max_retries:
                        ...
                        processed_error = await handler.handle_error_async(retry_error, context, suppress_errors)
                        if not suppress_errors and processed_error:
                            raise processed_error from e
