...
class AsyncConfigLoader:
            ...
            if file_path.suffix == ".encrypted" or ".encrypted" in str(file_path):
                if self._secure_loader:
                    ...
                    plain_text = await asyncio.to_thread(
                        self._secure_loader.decrypt_file, file_path
                    )
    ...
    async def load_config_async(
        ...
        configs = await asyncio.gather(
            *[self.load_file_async(path) for path in existing_paths],
        ...
        if self._enable_hot_reload:
            for path in existing_paths:
                await self._watch_file(path)

        return merged_config

    async def load_configs_concurrent_async(
        self,
        app_configs: List[Tuple[str, Path]]
        ...
        """
        configs = await asyncio.gather(
            *[self.load_config_async(app_name, root) for app_name, root in app_configs]
        )

        return {
            app_configs[i][0]: config
            for i, config in enumerate(configs)
        }

    async def _watch_file(self, file_path: Path):
    ...
    async def _handle_file_change(self, file_path: str):
        ...
        async with self._lock:
            ...
            keys_to_clear = [
                ...
                if path.name in key or str(path) in key
            ]
    ...
    async def clear_cache(self, app_name: Optional[str] = None):
...
async def get_async_config_loader(
    enable_hot_reload: bool = False,
    master_key: Optional[str] = None
) -> AsyncConfigLoader:
    ...
    if _global_async_loader is None:
        ...
        _global_async_loader = AsyncConfigLoader(
