...
def setup_logging(
    ...
    if json_format and HAS_JSON_LOGGER:
        ...
        if json_format and not HAS_JSON_LOGGER:
            logger.warning("JSON logging requested but python-json-logger not available, falling back to standard format")
    ...
    logging.info(f"Logging configured for '{name}' at level {log_level}. File logging: {log_to_file}, JSON format: {json_format and HAS_JSON_LOGGER}")
