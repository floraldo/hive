...
class GoldenRuleVisitor(ast.NodeVisitor):
    ...
    def add_violation(self, rule_id: str, rule_name: str, line_num: int, 
                     message: str, severity: str = "error"):
        """Add a violation if not suppressed"""
        if (line_num in self.context.suppressions and 
            rule_id in self.context.suppressions[line_num]):
    ...
    def _validate_error_handling_standards(self, node: ast.ClassDef):
        ...
        if any(isinstance(base, ast.Name) and base.id.endswith('Error') 
...
class EnhancedValidator:
    ...
    def _create_file_context(self, file_path: Path) -> FileContext:
        ...
        is_cli_file = ('cli.py' in str(file_path) or 
                      'secure_config.py' in str(file_path) or
                      '__main__' in content)
