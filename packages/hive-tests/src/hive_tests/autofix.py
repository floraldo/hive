    ...
    def fix_all_violations(self, target_rules: Optional[Set[str]] = None) -> List[AutofixResult]:
        ...
        for py_file in python_files:
            try:
                ...
                self.results.append(AutofixResult(
                    file_path=py_file,
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.AsyncFunctionDef):
                    ...
                    if matches > 0:
                        ...
                        changes_made.append(f"Renamed async function '{old_name}' to '{new_name}' ({matches} occurrences)")
...
def main():
    ...
    parser.add_argument("--rules", nargs="+", 
                       help="Specific rules to fix (e.g., rule-14 rule-9)")
