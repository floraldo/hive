...
class TestDirectDependencyViolations:
    ...
    def test_no_violations(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Create valid structure: module_a doesn't import module_b
            (tmpdir_path / "module_a.py").write_text("# Module A\n")
            (tmpdir_path / "module_b.py").write_text("# Module B\n")

            rule = DependencyRule(
                name="A cannot depend on B",
                source_pattern="module_a",
                target_pattern="module_b",
                rule_type=RuleType.CANNOT_DEPEND_ON,
    ...
    def test_direct_violation(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Create violation: module_a imports module_b
...
class TestTransitiveDependencyViolations:
    ...
    def test_transitive_violation_two_hops(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Create transitive violation: A → B → C
            (tmpdir_path / "module_a.py").write_text("import module_b\n")
            (tmpdir_path / "module_b.py").write_text("import module_c\n")
            (tmpdir_path / "module_c.py").write_text("# Module C\n")

            rule = DependencyRule(
                name="A cannot transitively depend on C",
                source_pattern="module_a",
                target_pattern="module_c",
                rule_type=RuleType.CANNOT_DEPEND_ON,
    ...
    def test_transitive_violation_three_hops(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Create deep transitive violation: A → B → C → D
            (tmpdir_path / "module_a.py").write_text("import module_b\n")
            (tmpdir_path / "module_b.py").write_text("import module_c\n")
            ...
            rule = DependencyRule(
                name="A cannot transitively depend on D",
                source_pattern="module_a",
                target_pattern="module_d",
                rule_type=RuleType.CANNOT_DEPEND_ON,
                check_transitive=True,
            )
            validator.add_rule(rule)
...
class TestPatternMatching:
    ...
    def test_directory_pattern_matching(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            rule = DependencyRule(
                name="Package A cannot depend on Package B",
                source_pattern="packages/pkg_a/**",
                target_pattern="packages/pkg_b/**",
                rule_type=RuleType.CANNOT_DEPEND_ON,
            )
            validator.add_rule(rule)
            ...
            # Should detect violation based on file paths
            assert len(violations) >= 0  # Pattern matching is file-path based
...
class TestViolationReporting:
    ...
    def test_violation_string_transitive(self) -> None:
        ...
        violation = Violation(
            ...
            dependency_path=["module_a", "module_b", "module_c"],
        )
...
class TestGraphBuilding:
    ...
    def test_graph_caching(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Second call should return same graph
...
class TestPredefinedRules:
    ...
    def test_predefined_rules_exist(self) -> None:
        ...
        # Check packages → apps rule exists
        packages_rule = next(
            (
                r
                for r in HIVE_ARCHITECTURAL_RULES
                if "packages" in r.source_pattern.lower()
                and "apps" in r.target_pattern.lower()
            ),
            None,
        )
