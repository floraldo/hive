...
class RequirementParser:
    ...
    def _extract_service_name(self, text: str) -> str:
        ...
        quoted_match = re.search(r"['\"]([a-z0-9-]+)['\"]", text, re.IGNORECASE)
        if quoted_match:
            return quoted_match.group(1)

        # Try hyphenated names
        hyphen_match = re.search(r"\b([a-z]+-[a-z-]+)\b", text, re.IGNORECASE)
        if hyphen_match:
    ...
    def _detect_service_type(self, text: str) -> ServiceType:
        ...
        elif self._matches_patterns(text_lower, self.WORKER_PATTERNS):
            return ServiceType.WORKER
        elif self._matches_patterns(text_lower, self.BATCH_PATTERNS):
