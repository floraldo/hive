...
class TestClimateResponseValidation:
    ...
    def test_response_has_all_required_fields(self, test_config):
...
class TestJobManagerRedis:
    ...
    def test_job_manager_fallback_without_redis(self):
        ...
        with patch(
            "EcoSystemiser.profile_loader.climate.job_manager.REDIS_AVAILABLE", False
        ):
            manager = JobManager()
            ...
            success = manager.update_job_status(
                job_id, JobStatus.COMPLETED, result={"output": "result"}
            )
    ...
    def test_time_gaps_with_explicit_frequency(self):
        ...
        gap_issue = next(
            (
                i
                for i in issues
                if "missing" in i.message.lower() or "gap" in i.message.lower()
            ),
            None,
        )
