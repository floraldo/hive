...
class TestFullSystemIntegration:
    ...
    def test_complete_climate_data_workflow(self):
    ...
    def test_adapter_to_processing_integration(self):
        ...
        ds = xr.Dataset({
            'temp_air': (['time'], 20 + 10 * np.sin(np.arange(168) * 2 * np.pi / 24) + np.random.randn(168) * 2),
            'ghi': (['time'], np.maximum(0, 400 * np.maximum(0, np.sin(np.arange(168) * 2 * np.pi / 24)) + np.random.randn(168) * 50)),
            'wind_speed': (['time'], np.maximum(0, 5 + np.random.randn(168) * 2)),
            'rel_humidity': (['time'], np.clip(65 + np.random.randn(168) * 15, 0, 100))
        }, coords={'time': time})
...
class TestApiIntegration:
    ...
    def test_service_real_integration(self):
        ...
        test_ds = xr.Dataset({
            'temp_air': (['time'], 15 + 5 * np.sin(np.arange(24) * 2 * np.pi / 24) + np.random.randn(24) * 0.5)
