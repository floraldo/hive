...
class GoldenMicrogridValidator:
    ...
    def validate_energy_balance(self, system, results: dict[str, Any], domain: str='electrical') -> dict[str, Any]:
        ...
        balance_results = {'domain': domain, 'timesteps_checked': system.N, 'max_imbalance': 0.0, 'total_imbalance': 0.0, 'balance_violations': [], 'passed': False}
        ...
        for t in range(system.N):
            ...
            if imbalance > self.tolerance:
                balance_results['balance_violations'].append({'timestep': t, 'sources': sources, 'sinks': sinks, 'imbalance': imbalance})
        balance_results['passed'] = balance_results['max_imbalance'] < self.tolerance and len(balance_results['balance_violations']) == 0
        logger.info(f"{domain.title()} energy balance: max_imbalance={balance_results['max_imbalance']:.2e}, ", f"violations={len(balance_results['balance_violations'])}")
    ...
    def validate_physics_constraints(self, system, results: dict[str, Any]) -> dict[str, Any]:
        ...
        physics_results = {'storage_bounds_violations': [], 'flow_negative_violations': [], 'capacity_violations': [], 'passed': False}
            ...
            if comp.type == 'storage' and hasattr(comp, 'E'):
                ...
                for t in range(system.N):
                    ...
                    if soc < -self.tolerance or soc > 1.0 + self.tolerance:
                        physics_results['storage_bounds_violations'].append({'component': comp_name, 'timestep': t, 'soc': soc, 'energy': comp.E[t], 'capacity': E_max})
        for flow_key, flow_data in system.flows.items():
            for t in range(system.N):
                flow_value = flow_data['value'][t] if hasattr(flow_data['value'], '__getitem__') else 0.0
                if flow_value < -self.tolerance:
                    physics_results['flow_negative_violations'].append({'flow': flow_key, 'timestep': t, 'value': flow_value})
        physics_results['passed'] = len(physics_results['storage_bounds_violations']) == 0 and len(physics_results['flow_negative_violations']) == 0 and (len(physics_results['capacity_violations']) == 0)
        logger.info(f"Physics constraints: storage_violations={len(physics_results['storage_bounds_violations'])}, flow_violations={len(physics_results['flow_negative_violations'])}")
        return physics_results

    def validate_system_behavior(self, system, results: dict[str, Any]) -> dict[str, Any]:
        """Validate expected system behavior patterns."""
        behavior_results = {'solar_priority_check': False, 'storage_cycling_check': False, 'thermal_coupling_check': False, 'peak_shaving_check': False, 'passed': False}
        solar_flows = ({},)
        grid_flows = {}
        for flow_key, flow_data in system.flows.items():
        ...
        if solar_flows and grid_flows:
            ...
            if solar_generation > 0:
                solar_priority_ratio = solar_generation / (solar_generation + grid_import + 1e-06)
        ...
        behavior_results['passed'] = behavior_results['solar_priority_check'] and behavior_results['storage_cycling_check'] and behavior_results['thermal_coupling_check']
        logger.info(f"System behavior: solar_priority={behavior_results['solar_priority_check']}, storage_cycling={behavior_results['storage_cycling_check']}, thermal_coupling={behavior_results['thermal_coupling_check']}")
    ...
    def validate_solver_performance(self, solve_time: float, memory_usage: float=0.0) -> dict[str, Any]:
        ...
        performance_results = {'solve_time': solve_time, 'memory_usage': memory_usage, 'time_acceptable': solve_time < 5.0, 'memory_acceptable': memory_usage < 100.0, 'passed': False}
        performance_results['passed'] = performance_results['time_acceptable'] and performance_results['memory_acceptable']
    ...
    def run_comprehensive_validation(self, system, solver_results, solve_time: float) -> dict[str, Any]:
        ...
        validation_results = {'timestamp': time.time(), 'system_id': system.system_id, 'timesteps': system.N, 'component_count': len(system.components), 'flow_count': len(system.flows)}
        ...
        validation_results['overall_passed'] = all([validation_results['electrical_balance']['passed'], validation_results['thermal_balance']['passed'], validation_results['physics_constraints']['passed'], validation_results['system_behavior']['passed'], validation_results['performance']['passed']])
    ...
    def calculate_total_cost(self, system, result) -> float:
        ...
        for _flow_key, flow_data in system.flows.items():
            if 'Grid' in flow_data['target'] and 'GRID' in flow_data['target']:
                ...
                if grid_comp and hasattr(grid_comp, 'params'):
                    export_tariff = getattr(grid_comp.params.technical, 'export_tariff', 0.1)
