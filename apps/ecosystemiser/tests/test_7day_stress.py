...
def create_7day_system():
    ...
    demand_profile = []
    for day in range(7):
        solar_factor = 1.0 + 0.2 * np.sin(day * np.pi / 3)
        demand_factor = 1.0 + 0.1 * np.cos(day * np.pi / 4)
        if day >= 5:
            demand_factor *= 0.8
    ...
    logger.info(f'Created 7-day profiles: solar range {np.min(solar_profile):.3f}-{np.max(solar_profile):.3f}, ', f'demand range {np.min(demand_profile):.3f}-{np.max(demand_profile):.3f}')
    ...
    battery_params = BatteryParams(technical=BatteryTechnicalParams(capacity_nominal=15.0, max_charge_rate=6.0, max_discharge_rate=6.0, efficiency_roundtrip=0.95, initial_soc_pct=0.5, fidelity_level=FidelityLevel.SIMPLE))
    battery = Battery('Battery', battery_params, N)
    solar_params = SolarPVParams(technical=SolarPVTechnicalParams(capacity_nominal=40.0, efficiency_nominal=1.0, fidelity_level=FidelityLevel.SIMPLE))
...
def validate_long_term_stability(system, tolerance=1e-05):
    """Validate long-term numerical stability."""
    stability_results = {'energy_drift_check': True, 'storage_bounds_check': True, 'flow_consistency_check': True, 'numerical_stability': True}
    ...
    energy_drift = max_daily_imbalance > tolerance * 100
    ...
    stability_results['numerical_stability'] = all([stability_results['energy_drift_check'], stability_results['storage_bounds_check'], stability_results['flow_consistency_check']])
...
def analyze_weekly_patterns(system):
    ...
    if battery_comp and hasattr(battery_comp, 'E'):
        ...
        analysis['cycling_analysis'] = {'weekly_energy_range': float(weekly_range), 'equivalent_cycles': float(cycles), 'initial_soc': float(battery_comp.E[0] / battery_comp.E_max), 'final_soc': float(battery_comp.E[-1] / battery_comp.E_max)}
    return analysis
...
def run_7day_stress_test():
    ...
    try:
        ...
        results.update({'solver_status': result.status, 'solve_time': float(solve_time), 'initial_memory_mb': float(initial_memory), 'peak_memory_mb': float(peak_memory), 'memory_increase_mb': float(peak_memory - initial_memory), 'solver_success': result.status == 'optimal'})
        if result.status == 'optimal':
            ...
            results.update({'stability_analysis': stability, 'pattern_analysis': patterns, 'validation_passed': stability['numerical_stability']})
            logger.info('\n' + '=' * 60)
            logger.info('7-DAY STRESS TEST SUMMARY')
            logger.info('=' * 60)
            logger.info(f'Solve time: {solve_time:.2f}s')
            logger.info(f'Memory usage: {initial_memory:.1f} → {peak_memory:.1f} MB (+{peak_memory - initial_memory:.1f})')
            ...
            logger.info(f"Weekly totals: {weekly['total_solar']:.0f} kWh solar, {weekly['total_demand']:.0f} kWh demand")
            logger.info(f"Grid interaction: {weekly['total_import']:.0f} kWh import, {weekly['total_export']:.0f} kWh export")
            cycling = patterns['cycling_analysis']
            logger.info(f"Battery cycling: {cycling['equivalent_cycles']:.2f} cycles, SOC {cycling['initial_soc']:.1%} → {cycling['final_soc']:.1%}")
...
def main():
    ...
    if results.get('solve_time'):
        logger.info(f"Performance: {results['solve_time']:.2f}s solve time, ", f"{results.get('memory_increase_mb', 0):.1f} MB memory increase")
