...
def load_yearly_profile(profile_name: str) -> np.ndarray:
    ...
    profile_path = Path(__file__).parent.parent / 'data' / 'yearly_scenarios' / 'profiles' / f'{profile_name}_yearly.csv'
...
def create_yearly_system() -> System:
    ...
    grid_params = GridParams(technical=GridTechnicalParams(capacity_nominal=800.0, import_tariff=0.25, feed_in_tariff=0.08, fidelity_level=FidelityLevel.SIMPLE))
    grid = Grid('Grid', grid_params, N)
    battery_params = BatteryParams(technical=BatteryTechnicalParams(capacity_nominal=300.0, max_charge_rate=150.0, max_discharge_rate=150.0, efficiency_roundtrip=0.95, initial_soc_pct=0.5, fidelity_level=FidelityLevel.SIMPLE))
...
def validate_energy_balance(system: System, tolerance: float=0.001) -> dict[str, float]:
...
def analyze_yearly_performance(system: System) -> dict[str, Any]:
    ...
    analysis = {'solar_generation_mwh': solar_total / 1000, 'total_demand_mwh': demand_total / 1000, 'grid_import_mwh': grid_import_total / 1000, 'grid_export_mwh': grid_export_total / 1000, 'net_grid_mwh': (grid_import_total - grid_export_total) / 1000, 'self_consumption_ratio': min(1.0, demand_total / solar_total) if solar_total > 0 else 0, 'self_sufficiency_ratio': min(1.0, (solar_total - grid_export_total) / demand_total) if demand_total > 0 else 0, 'battery_cycles': equivalent_cycles, 'battery_avg_soc': avg_soc, 'battery_range_kwh': yearly_range}
    return analysis
...
def test_rule_based_yearly():
    ...
    try:
        ...
        if result.status == 'optimal':
            ...
            results.update({'solver_status': result.status, 'solve_time_seconds': solve_time, 'memory_usage': {'initial_mb': initial_memory, 'peak_mb': solve_memory, 'increase_mb': solve_memory - initial_memory}, 'energy_balance': balance_results, 'yearly_performance': performance, 'validation_passed': balance_results['energy_balance_ok']})
            logger.info('\\n' + '=' * 60)
...
def test_milp_yearly():
    ...
    try:
        ...
        if result.status == 'optimal':
            balance_results = (validate_energy_balance(system),)
            performance = analyze_yearly_performance(system)
            results.update({'solver_status': result.status, 'solve_time_seconds': solve_time, 'memory_usage': {'initial_mb': initial_memory, 'peak_mb': solve_memory, 'increase_mb': solve_memory - initial_memory}, 'energy_balance': balance_results, 'performance': performance, 'validation_passed': balance_results['energy_balance_ok']})
            logger.info('\\n' + '=' * 60)
