...
from ecosystemiser.system_model.components.energy.battery import (
    ...
    BatteryOptimizationStandard,
    BatteryParams,
    BatteryPhysicsSimple,
    BatteryPhysicsStandard,
)
...
def test_battery_complete_strategy():
    ...
    assert (
        BatteryOptimizationStandard is not None
    ), "BatteryOptimizationStandard missing"
    logger.info("   ✅ All 4 strategy classes exist")

    # Test 2: Inheritance chains are correct
    logger.info("\n2. Checking inheritance chains...")
    assert issubclass(
        BatteryPhysicsStandard, BatteryPhysicsSimple
    ), "BatteryPhysicsStandard should inherit from BatteryPhysicsSimple"
    assert issubclass(
        BatteryOptimizationStandard, BatteryOptimizationSimple
    ...
    assert not isinstance(
        physics_simple, BatteryPhysicsStandard
    ...
    assert not isinstance(
        optimization_simple, BatteryOptimizationStandard
    ...
    assert isinstance(
        physics_standard, BatteryPhysicsSimple
    ...
    assert isinstance(
        optimization_standard, BatteryOptimizationSimple
    ...
    assert hasattr(
        physics_simple, "rule_based_update_state"
    ), "Physics strategy must have rule_based_update_state"
    assert hasattr(
        optimization_simple, "set_constraints"
    ...
    physics_simple = battery_simple._get_physics_strategy()
    energy_simple = physics_simple.rule_based_update_state(
        0, initial_energy, charge, discharge
    )

    # STANDARD physics (with self-discharge)
    battery_standard.technical.self_discharge_rate = 0.01
    physics_standard = battery_standard._get_physics_strategy()
    energy_standard = physics_standard.rule_based_update_state(
        0, initial_energy, charge, discharge
    )

    assert (
        energy_simple == initial_energy
    ), "SIMPLE physics should have no self-discharge"
    assert (
        energy_standard < initial_energy
    ), "STANDARD physics should have self-discharge losses"
    assert (
        abs(initial_energy - energy_standard - 0.1) < 0.001
