...
from ecosystemiser.services.simulation_service import SimulationService, SimulationResult
...
class TestDiscoveryEngineE2E:
    ...
    def test_genetic_algorithm_workflow(self, temp_workspace, energy_system_config,
                                       profile_config):
        ...
        def mock_simulation(config):
            ...
            solar_capacity = config['system_config']['components'][0]['technical']['power_capacity_nominal']
            battery_capacity = config['system_config']['components'][1]['technical']['energy_capacity_nominal']
            ...
            renewable_fraction = min(0.95, solar_capacity / 50 * 0.7 + battery_capacity / 100 * 0.3)
    ...
    def test_monte_carlo_workflow(self, temp_workspace, energy_system_config,
                                 profile_config):
        ...
        def mock_simulation(config):
            ...
            solar_eff = config['system_config']['components'][0]['technical']['efficiency']
            elec_price = config['system_config']['components'][2]['economic']['electricity_price']
            annual_demand = config['system_config']['components'][3]['profile']['annual_consumption']
    ...
    def test_report_generation(self, temp_workspace):
        ...
        plots['convergence'] = plot_factory.create_ga_convergence_plot(ga_result['best_result'])

        # Mock uncertainty distribution
        plots['uncertainty'] = plot_factory.create_uncertainty_distribution_plot(mc_result['best_result'])
    ...
    def test_performance_metrics(self, temp_workspace):
        ...
        assert 'num_simulations' in result
        expected_sims = study_config.algorithm_config['population_size'] * \
        ...
        logger.info(f"Performance: {perf_report['evaluations_per_second']:.1f} evals/sec")
