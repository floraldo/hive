...
def test_water_grid_initialization():
    ...
    params = WaterGridParams(
        max_supply_m3h=10.0,
        water_price_per_m3=1.5,
        supply_reliability=0.99
    )
...
def test_water_storage_operation():
    ...
    outflow, inflow = storage.rule_based_operation(
        water_demand=0.0,  # No demand
        water_supply=1.5,  # Supply available
        t=0
    )
    ...
    # Test drawing
    outflow, inflow = storage.rule_based_operation(
        water_demand=1.0,  # Demand exists
        water_supply=0.0,  # No supply
        t=1
    )
