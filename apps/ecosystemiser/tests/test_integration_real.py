...
class TestRealServiceIntegration:
    ...
    def test_complete_workflow_with_real_data(self):
        ...
        ghi = np.maximum(
            0, 500 * np.sin((hours_array - 6) * np.pi / 12)
        )  # Zero at night
        ghi[hours_array < 6] = 0
        ...
        ds = xr.Dataset(
            ...
            coords={"time": times},
            attrs={"location": "Test Site", "latitude": 51.5, "longitude": -0.1},
        )

        # Apply real validation
        qc_report = validate_complete(
            ds, source="test", validation_level="comprehensive"
        )
        ...
        ds_cleaned, final_report = apply_quality_control(
            ds, source="test", comprehensive=True
        )
    ...
    def test_real_processing_pipeline(self):
        ...
        from ecosystemiser.profile_loader.climate.processing.resampling import (
            resample_dataset,
        )
        ...
        ds = xr.Dataset(
            {
                "temp_air": (
                    ["time"],
                    15 + 10 * np.sin(np.arange(168) * 2 * np.pi / 24),
                ),
                "ghi": (
                    ["time"],
                    np.maximum(0, 500 * np.sin(np.arange(168) * np.pi / 24)),
                ),
            },
            coords={"time": times},
        )
...
class TestRealJobManager:
    ...
    def test_job_manager_real_workflow(self):
        ...
        success = manager.update_job_status(job_id, JobStatus.PROCESSING, progress=50)
        assert success
        ...
        success = manager.update_job_status(
            job_id, JobStatus.COMPLETED, result=result_data, progress=100
        )
        assert success
...
class TestRealValidation:
    ...
    def test_real_data_quality_issues(self):
        ...
        if len(valid_temps) > 0:
            ...
            assert (
                extreme_low_count < original_extreme_low
                or extreme_high_count < original_extreme_high
            )
...
class TestRealFileOperations:
    ...
    def test_real_cache_operations(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            times = pd.date_range("2023-01-01", periods=24, freq="H")
            ds = xr.Dataset(
                {"temp_air": (["time"], np.random.randn(24))}, coords={"time": times}
            )
            ...
            np.testing.assert_array_almost_equal(
                ds["temp_air"].values, ds_loaded["temp_air"].values
            )
