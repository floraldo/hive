...
class TestValidation:
    ...
    def create_sample_dataset(self) -> xr.Dataset:
        ...
        temp_air = (
            15 + 10 * np.sin(np.arange(100) * 2 * np.pi / 24) + np.random.randn(100) * 2
        )
        ghi = np.maximum(
            0,
            300
            + 200 * np.sin(np.arange(100) * 2 * np.pi / 24)
    ...
    def test_apply_quality_control(self):
        ...
        ds_qc, report = apply_quality_control(
            ds, source="test_data", comprehensive=True
        )
    ...
    def test_apply_corrections(self):
        ...
        # Apply corrections
        ds_corrected = apply_corrections(ds, report, spike_filter=True, gap_fill=True)
    ...
    def test_qc_report_functionality(self):
        ...
        report = QCReport(
            dataset_id="test_dataset", timestamp=datetime.now().isoformat()
        )
...
class TestProcessingPipeline:
    ...
    def test_pipeline_execution(self, test_config):
        ...
        ds = xr.Dataset(
            {"temp_air": (["time"], np.random.randn(24) + 15)}, coords={"time": time}
        )
...
class TestDataIntegrity:
    ...
    def test_no_data_corruption_through_pipeline(self):
        ...
        ds = xr.Dataset(
            {"temp_air": (["time"], original_values)}, coords={"time": time}
        )
