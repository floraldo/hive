...
class TestProductionIntegration:
    ...
    def yearly_profiles(self) -> dict[str, np.ndarray]:
        ...
        if solar_path.exists():
            ...
            profiles['solar'] = daily_pattern * seasonal_factor * 5.0
        ...
        import_prices[peak_hours] = base_price * 1.5
        import_prices[off_peak] = base_price * 0.7
        profiles['import_price'] = import_prices
        profiles['export_price'] = import_prices * 0.4
        return profiles
    ...
    def test_24hour_real_system(self, yearly_profiles):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            run_dir = results_io.save_results(system, 'test_24h', Path(tmpdir), metadata={'test_type': '24hour', 'solver': 'rule_based', 'solve_time': solve_time})
    ...
    def test_weekly_simulation(self, yearly_profiles):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            results_io = (SimpleResultsIO(),)
            run_dir = results_io.save_results(system, 'test_week', Path(tmpdir), metadata={'test_type': 'weekly', 'solver': 'rule_based', 'solve_time': solve_time})
            loaded = results_io.load_results(run_dir)
    ...
    def test_yearly_rolling_horizon(self, yearly_profiles):
        ...
        system.add_flow('solar_to_battery', 'SOLAR', 'BATTERY')
        system.add_flow('battery_to_demand', 'BATTERY', 'DEMAND')
        solver = RollingHorizonMILPSolver(horizon_hours=168, overlap_hours=24, warmstart=True)
        start_time = (time.time(),)
        result = (solver.solve(system),)
        solve_time = time.time() - start_time
        assert result['status'] == 'optimal'
