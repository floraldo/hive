...
def create_water_system():
    ...
    water_grid = WaterGrid(
        name="WaterGrid", params=WaterGridParams()  # Use defaults, then override
    )
    ...
    water_storage = WaterStorage(
        name="WaterStorage", params=WaterStorageParams()  # Use defaults, then override
    )
    ...
    water_demand_load = WaterDemand(
        name="WaterDemand", params=WaterDemandParams()  # Use defaults, then override
    )
    ...
    if result.status == "optimal":
        ...
        if hasattr(water_storage, "W") and water_storage.W is not None:
            if hasattr(water_storage.W, "value"):
                ...
                logger.info(
                    f"Water storage range: {water_storage.W.value.min():.2f} - {water_storage.W.value.max():.2f} m³"
                )
            elif isinstance(water_storage.W, np.ndarray):
                # Already extracted as numpy array
                logger.info(
                    f"Water storage range: {water_storage.W.min():.2f} - {water_storage.W.max():.2f} m³"
                )
