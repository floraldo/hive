...
def create_fidelity_test_system(fidelity_level: FidelityLevel):
    ...
    if fidelity_level >= FidelityLevel.STANDARD:
        heat_buffer.technical.heat_loss_coefficient = (
            0.002  # 0.2% per hour thermal loss
        )
...
def test_rule_based_fidelity_comparison():
    ...
    for comp in [simple_water_storage, standard_water_storage]:
        ...
        comp.water_level[0] = (
            comp.technical.initial_soc_pct * comp.technical.capacity_nominal
        )

    # Simulate operation with no charging/discharging to see pure fidelity effects
    logger.info(
        "Simulating 12 hours with zero charge/discharge to isolate fidelity effects..."
    )
    ...
    for t in range(N):
        ...
        simple_heat_buffer.E[t + 1] = (
            simple_heat_buffer.physics.rule_based_update_state(
                t, simple_heat_buffer.E[t], 0.0, 0.0
            )
        )
        standard_heat_buffer.E[t + 1] = (
            standard_heat_buffer.physics.rule_based_update_state(
                t, standard_heat_buffer.E[t], 0.0, 0.0
            )
        )

        # Water storage state evolution using physics strategies directly
        simple_water_storage.water_level[t + 1] = (
            simple_water_storage.physics.rule_based_update_state(
                t, simple_water_storage.water_level[t], 0.0, 0.0
            )
        )
        standard_water_storage.water_level[t + 1] = (
            standard_water_storage.physics.rule_based_update_state(
    ...
    logger.info(
        f"  Difference: {simple_final - standard_final:.2f} kWh lost to self-discharge"
    )
    ...
    logger.info(
        f"  Difference: {simple_heat_final - standard_heat_final:.2f} kWh lost to ambient"
    )
    ...
    logger.info(
        f"  STANDARD: {standard_water_final:.2f} m³ (temperature-enhanced evaporation)"
    )
    logger.info(
        f"  Difference: {simple_water_final - standard_water_final:.2f} m³ additional evaporation"
    )
...
def test_optimization_fidelity_comparison():
    ...
    if simple_result.status == "optimal" and standard_result.status == "optimal":
        ...
        logger.info(
            f"  Cost Difference: ${cost_diff:.2f} (STANDARD accounts for losses)"
        )
    ...
    return {
        ...
        "simple_cost": (
            simple_result.objective_value if simple_result.status == "optimal" else None
        ),
        "standard_cost": (
            standard_result.objective_value
...
def main():
    ...
    if rule_based_results["heat_loss"] > 0:
        logger.info(
            f"🔥 Heat buffer thermal loss: {rule_based_results['heat_loss']:.2f} kWh lost in 12h"
        )
    if rule_based_results["water_loss"] > 0:
        logger.info(
            f"💧 Water enhanced evaporation: {rule_based_results['water_loss']:.2f} m³ lost in 12h"
        )
