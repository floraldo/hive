...
def create_comprehensive_system():
    ...
    solar_profile[6:18] = [0.1, 0.2, 0.4, 0.6, 0.8, 0.9,
...
def test_component_inventory():
    ...
    for comp_name, params_class in test_components:
        try:
            ...
            assert hasattr(instance, 'medium'), f"{comp_name} missing 'medium' attribute"
            assert hasattr(instance, 'technical'), f"{comp_name} missing 'technical' attribute"
...
def test_comprehensive_optimization():
    ...
    for name, comp in system.components.items():
        ...
        logger.info(f"  - {name:15} ({comp.type:12}) [{comp.medium:11}] Fidelity: {fidelity}")
    ...
    if result.status == 'optimal':
        ...
        for comp in system.components.values():
            has_vars = False
            for attr in ['E_opt', 'P_cha', 'P_dis', 'P_out', 'Q_in', 'Q_out',
                        'P_draw', 'P_feed', 'Q_import', 'Q_export', 'V_water']:
        ...
        logger.info(f"Components with optimization variables: {components_with_vars}/{len(system.components)}")
        return True
    else:
