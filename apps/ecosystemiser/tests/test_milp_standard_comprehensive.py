...
def create_comprehensive_test_system(fidelity_level: FidelityLevel):
    ...
    battery.technical.self_discharge_rate = 0.01  # 1% per hour (increased for visibility)
    ...
    heat_buffer.technical.heat_loss_coefficient = 0.02  # 2% per hour (increased for visibility)
...
def analyze_component_differences(simple_system, standard_system):
    ...
    if hasattr(battery_simple, 'E_opt') and battery_simple.E_opt is not None:
        if hasattr(battery_simple.E_opt, 'value') and battery_simple.E_opt.value is not None:
    ...
    if hasattr(water_simple, 'V_water') and water_simple.V_water is not None:
        if hasattr(water_simple.V_water, 'value') and water_simple.V_water.value is not None:
    ...
    if hasattr(solar_simple, 'P_out') and solar_simple.P_out is not None:
        if hasattr(solar_simple.P_out, 'value') and solar_simple.P_out.value is not None:
    ...
    if hasattr(heat_simple, 'H_in') and heat_simple.H_in is not None:
        if hasattr(heat_simple.H_in, 'value') and heat_simple.H_in.value is not None:
            ...
            differences['HeatDemand'] = {
                ...
                'flexibility_used': np.max(standard_deviation) > np.max(simple_deviation),
...
def test_comprehensive_milp_standard():
    ...
    components_with_differences = sum(1 for d in differences.values()
                                     if ('difference' in d and abs(d['difference']) > 0.01) or
                                        ('flexibility_used' in d and d['flexibility_used']))

    logger.info("\n" + "-"*50)
    logger.info("VALIDATION SUMMARY")
    logger.info("-"*50)
    logger.info(f"Components with STANDARD effects: {components_with_differences}/{len(differences)}")
    logger.info(f"Total cost increase: {percent_difference:.1f}%")

    if components_with_differences >= 4:  # At least 4 components should show differences
        logger.info("\n✅ SUCCESS: MILP STANDARD fidelity is comprehensively implemented!")
        logger.info("All major component types show realistic physics effects in optimization.")
        return True
    else:
