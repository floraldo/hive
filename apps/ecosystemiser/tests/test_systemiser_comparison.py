...
def run_original_systemiser_milp():
    ...
    results = {
        ...
        "battery_energy": (
            battery.E.value.tolist() if battery.E.value is not None else []
        ),
        "grid_import": (
            grid.flows["source"]["P_draw"]["value"].value.tolist()
            if grid.flows["source"]["P_draw"]["value"].value is not None
            else []
        ),
        "grid_export": (
            grid.flows["sink"]["P_feed"]["value"].value.tolist()
            if grid.flows["sink"]["P_feed"]["value"].value is not None
            else []
        ),
        "solar_output": (
            solar.flows["source"]["P_out"]["value"].value.tolist()
            if solar.flows["source"]["P_out"]["value"].value is not None
            else []
        ),
        "demand_input": (
            demand.flows["sink"]["P_in"]["value"].value.tolist()
            if demand.flows["sink"]["P_in"]["value"].value is not None
            else []
        ),
    }
...
def run_ecosystemiser_milp():
    ...
    solar = SolarPV(
        name="SolarPV", params=SolarPVParams(P_profile=solar_profile.tolist(), P_max=10)
    )
    system.add_component(solar)
    ...
    results = {
        ...
        "battery_energy": (
            battery.E.tolist()
            if hasattr(battery, "E") and battery.E is not None
            else []
        ),
        "grid_import": (
            grid.P_import.tolist()
            if hasattr(grid, "P_import") and grid.P_import is not None
            else []
        ),
        "grid_export": (
            grid.P_export.tolist()
            if hasattr(grid, "P_export") and grid.P_export is not None
            else []
        ),
        "solar_output": (
            solar.P_generation.tolist()
            if hasattr(solar, "P_generation") and solar.P_generation is not None
            else []
        ),
        "demand_input": (
            demand.P_consumption.tolist()
            if hasattr(demand, "P_consumption") and demand.P_consumption is not None
            else []
        ),
    }
...
def compare_results(orig_results, eco_results):
    ...
    if orig_results["battery_energy"] and eco_results["battery_energy"]:
        ...
        logger.info(
            f"  Original range: {orig_battery.min():.2f} - {orig_battery.max():.2f} kWh"
        )
        logger.info(
            f"  Eco range: {eco_battery.min():.2f} - {eco_battery.max():.2f} kWh"
        )
    ...
    if orig_results["grid_import"] and eco_results["grid_import"]:
        ...
        logger.info(
            f"  Difference: {abs(orig_import.sum() - eco_import.sum()):.4f} kWh"
        )
    ...
    if orig_results["grid_export"] and eco_results["grid_export"]:
        ...
        logger.info(
            f"  Difference: {abs(orig_export.sum() - eco_export.sum()):.4f} kWh"
        )
    ...
    if match:
        logger.info(
            "✅ VALIDATION SUCCESSFUL! Both systems produce equivalent results!"
        )
    else:
