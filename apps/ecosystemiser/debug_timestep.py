...
print("=== TIMESTEP-BY-TIMESTEP DEBUGGING ===")
print()
...
for t in range(24):
    ...
    if t == 0:
        ...
        print(f"  Initial battery state: {initial_state:.6f} kWh (E_init)")
    else:
        initial_state = battery.E[t-1]
        print(f"  Initial battery state: {initial_state:.6f} kWh (E[{t-1}])")
    ...
    print(f"  Charge flows: {', '.join(charge_flows) if charge_flows else 'None'}")
    print(f"  Discharge flows: {', '.join(discharge_flows) if discharge_flows else 'None'}")
    ...
    print(f"  Net change: {net_change:.6f} kWh (charge: {energy_gained:.6f}, discharge: {energy_lost:.6f})")
    
    # Print calculated final state
    print(f"  Calculated final state: {battery.E[t]:.6f} kWh (E[{t}])")
    print(f"  Golden final state: {golden_battery_states[t]:.6f} kWh")
    
    # Check for discrepancy
    diff = abs(battery.E[t] - golden_battery_states[t])
    if diff > 1e-6:
        print(f"  ❌ ERROR: Discrepancy of {diff:.9f} kWh detected!")
        print(f"  Expected: initial ({initial_state:.6f}) + net_change ({net_change:.6f}) = {initial_state + net_change:.6f}")
        print(f"  But E[{t}] = {battery.E[t]:.6f}")
        print("\n  FOUND THE BUG! The calculation is using the wrong initial state.")
        break
    else:
        print(f"  ✓ States match within tolerance")
