...
st.markdown(
    "**Interactive interface for exploring climate data from multiple sources**"
)
...
with st.sidebar:
    ...
    location_type = st.radio(
        "Input type:", ["Coordinates", "City Name"], horizontal=True
    )
    ...
    if location_type == "Coordinates":
        ...
        with col1:
            latitude = st.number_input(
                "Latitude", min_value=-90.0, max_value=90.0, value=38.7, step=0.01
            )
        with col2:
            longitude = st.number_input(
                "Longitude", min_value=-180.0, max_value=180.0, value=-9.1, step=0.01
            )
    ...
    if period_type == "Year":
        year = st.number_input(
            "Year", min_value=1980, max_value=2024, value=2023, step=1
        )
        period = {"year": year}
    else:
        col1, col2 = st.columns(2)
    ...
    resolution = st.selectbox(
        "Resolution", ["15min", "30min", "1H", "3H", "1D"], index=2
    )
    ...
    temp_vars = [
        v for v in CANONICAL_VARIABLES.keys() if "temp" in v or "dewpoint" in v
    ]
    solar_vars = [
        v
        for v in CANONICAL_VARIABLES.keys()
        if any(s in v for s in ["ghi", "dni", "dhi", "solar", "sunshine"])
    ]
    ...
    precip_vars = [
        v
        for v in CANONICAL_VARIABLES.keys()
        if any(p in v for p in ["precip", "rain", "snow"])
    ]
    other_vars = [
        v
        for v in CANONICAL_VARIABLES.keys()
    ...
    # Submit button
    submit = st.button(
        "🚀 Get Climate Profile", type="primary", use_container_width=True
    )
...
if submit:
    ...
    try:
        ...
        st.info(
            "💡 Try adjusting your query parameters or selecting a different data source."
        )
        st.stop()
...
if st.session_state.dataset is not None and st.session_state.api_response is not None:
    ...
    if plot_vars:
        ...
        for var in primary_vars:
            fig.add_trace(
                go.Scatter(x=df.index, y=df[var], mode="lines", name=var, yaxis="y")
            )

        # Add traces for secondary axis
        for var in secondary_vars:
            fig.add_trace(
                go.Scatter(x=df.index, y=df[var], mode="lines", name=var, yaxis="y2")
            )
        ...
        fig.update_layout(
            ...
            yaxis2=(
                dict(title="Solar Radiation (W/m²)", overlaying="y", side="right")
                if secondary_vars
                else None
            ),
            hovermode="x unified",
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📊 Raw Data", "📋 Manifest", "📈 Statistics", "💾 Download"]
    )
    ...
    with tab1:
        ...
        with col1:
            n_rows = st.number_input(
                "Show rows:", min_value=10, max_value=len(df), value=100, step=10
            )
    ...
    with tab3:
        ...
        if (
            hasattr(st.session_state.api_response, "stats")
            and st.session_state.api_response.stats
        ):
    ...
    with tab4:
            ...
            if hasattr(st.session_state.api_response, "manifest"):
                try:
                    ...
                    if isinstance(manifest_data, dict):
                        ...
                        json_str = json.dumps(
                            serializable_manifest, indent=2, default=str
                        )
                    else:
                        json_str = json.dumps(
                            {"manifest": str(manifest_data)}, indent=2
                        )
        ...
        st.markdown("---")
        st.caption(
            "💡 **Tip:** Parquet format is recommended for large datasets and maintains data types."
        )

# Footer
st.markdown("---")
st.caption(
    "🌍 EcoSystemiser Climate Dashboard | Built with Streamlit | [Documentation](https://github.com/yourusername/ecosystemiser)"
)
