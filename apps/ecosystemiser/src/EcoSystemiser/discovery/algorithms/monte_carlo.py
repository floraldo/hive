...
class MonteCarloEngine(BaseOptimizationAlgorithm):
    ...
    def evaluate_population(self, population: np.ndarray,
        ...
        if self.config.parallel_evaluation and self.config.max_workers > 1:
            with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
                futures = [executor.submit(fitness_function, sample)
    ...
    def update_population(self, population: np.ndarray,
                         evaluations: List[Dict[str, Any]]) -> np.ndarray:
    ...
    def _perform_uncertainty_analysis(self, samples: np.ndarray,
    ...
    def _calculate_sensitivity_indices(self, samples: np.ndarray,
                                     evaluations: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Calculate sensitivity indices using correlation analysis."""
        sensitivity = {}

        valid_indices = [i for i, eval_result in enumerate(evaluations)
                        if eval_result.get('valid', True)]
    ...
    def _perform_scenario_analysis(self, samples: np.ndarray,
                                 evaluations: List[Dict[str, Any]]) -> Dict[str, Any]:
                ...
                for scenario_name, scenario_def in scenario_definitions.items():
                    ...
                    if np.sum(scenario_indices) > 0:
                        ...
                        scenarios[obj_name][scenario_name] = {
                            ...
                            'parameter_means': [float(np.mean(scenario_samples[:, i]))
                                              for i in range(scenario_samples.shape[1])],
                            'parameter_stds': [float(np.std(scenario_samples[:, i]))
                                             for i in range(scenario_samples.shape[1])]
                        }

        return scenarios

    def _get_best_sample(self, samples: np.ndarray,
                        evaluations: List[Dict[str, Any]]) -> Optional[np.ndarray]:
        """Get best sample based on primary objective."""
        if not evaluations:
            return None

        if len(self.config.objectives) == 1:
            best_idx = min(range(len(evaluations)),
                          key=lambda i: evaluations[i].get('fitness',
                                                          evaluations[i].get('objectives', [float('inf')])[0]))
    ...
    def _get_best_fitness(self, evaluations: List[Dict[str, Any]]) -> float:
        ...
        if len(self.config.objectives) == 1:
            fitness_values = [eval_result.get('fitness',
                                            eval_result.get('objectives', [float('inf')])[0])
                            for eval_result in valid_evaluations]
    ...
    def _get_best_objectives(self, evaluations: List[Dict[str, Any]]) -> Optional[List[float]]:
        ...
        if len(self.config.objectives) == 1:
            best_eval = min(valid_evaluations,
                           key=lambda e: e.get('fitness',
                                             e.get('objectives', [float('inf')])[0]))
...
class UncertaintyAnalyzer:
    ...
    def run_uncertainty_analysis(self, fitness_function: Callable,
                                parameter_uncertainties: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def run_sensitivity_study(self, fitness_function: Callable,
