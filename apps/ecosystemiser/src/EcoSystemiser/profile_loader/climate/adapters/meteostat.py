...
class MeteostatAdapter(BaseAdapter):
    ...
    async def _fetch_raw(
        ...
        **kwargs
        ...
        # Fetch data
        self.logger.info(f"Fetching Meteostat data for {lat:.3f},{lon:.3f} from {start_date.date()} to {end_date.date()}")
    ...
    async def fetch(
        ...
        *,
        ...
        try:
            ...
            regular_vars = [v for v in variables if v in self.VARIABLE_MAPPING]
            special_vars = [v for v in variables if v in self.SPECIAL_VARIABLES]
            unavailable_vars = [v for v in variables if v not in self.VARIABLE_MAPPING and v not in self.SPECIAL_VARIABLES]
            
            if unavailable_vars:
                logger.warning(f"Variables not available in Meteostat: {unavailable_vars}")
            
            if not regular_vars and not special_vars:
                raise ValidationError(f"No supported variables requested. Available: {list(self.VARIABLE_MAPPING.keys()) + list(self.SPECIAL_VARIABLES.keys())}")
    ...
    def _process_special_variables(self, ds: xr.Dataset, df: pd.DataFrame, special_vars: List[str]) -> xr.Dataset:
        ...
        for var in special_vars:
            if var == 'cloud_cover' and 'coco' in df.columns:
                ...
                ghi_data = self._sunshine_to_ghi(df['tsun'].values, ds.time.values, 
                                               ds.attrs.get('latitude', 0))
                ds[var] = xr.DataArray(
                    ghi_data,
                    coords={"time": ds.time},
                    attrs=self._get_variable_attrs(var)
    ...
    def get_capabilities(self) -> AdapterCapabilities:
        ...
        return AdapterCapabilities(
            ...
            primary_variables=[
                "temp_air", "precip", "wind_speed",  # Best for basic met vars
