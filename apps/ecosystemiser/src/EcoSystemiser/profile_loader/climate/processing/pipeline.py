...
class ProcessingPipeline:
    ...
    def _setup_default_preprocessing(self):
        ...
        if gap_fill_enabled:
            self.add_preprocessing_step(
                ...
                smart_fill_gaps,
                config={
                    "max_linear_gap": profile_config.gap_fill_max_hours // 8 if profile_config and hasattr(profile_config, 'gap_fill_max_hours') else 3,
                    "max_pattern_gap": profile_config.gap_fill_max_hours if profile_config and hasattr(profile_config, 'gap_fill_max_hours') else 24,
    ...
    def execute(self, ds: xr.Dataset, 
               skip_preprocessing: bool = False,
               skip_postprocessing: bool = False) -> xr.Dataset:
        """
    ...
    def get_execution_report(self) -> Dict[str, Any]:
        ...
        return {
            "preprocessing": [r for r in self.execution_reports 
                            if r.get("stage") == "preprocessing"],
            "postprocessing": [r for r in self.execution_reports
                             if r.get("stage") == "postprocessing"],
