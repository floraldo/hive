    ...
    def validate_batch(
        ...
        for ds, identifier, source in datasets:
            ...
            try:
                ...
                error_report.add_issue(QCIssue(
...
def apply_corrections(
    ...
    critical_bounds_issues = [
        issue for issue in report.get_critical_issues() 
    ...
    if gap_fill:
        gap_issues = [issue for issue in report.issues if 'gap' in issue.type.lower() or 'missing' in issue.type.lower()]
        if gap_issues:
...
def fill_gaps(
    ...
    for var_name in ds_filled.data_vars:
        ...
        if n_gaps_remaining > 0:
            ...
            seasonal_median = data.groupby(
                [time_index.month, time_index.hour]
            ).median()
