...
class GapFiller:
    ...
    def fill_dataset(self, ds: xr.Dataset) -> Tuple[xr.Dataset, Dict]:
        ...
        logger.info(f"Gap filling complete: {total_filled}/{total_gaps} gaps filled "
    ...
    def _fill_similar_day_pattern(self, 
                                  data: xr.DataArray,
                                  prefer_clear: bool = False) -> xr.DataArray:
        """
        ...
        for i, t in enumerate(time_index):
            if np.isnan(values[i]):
                ...
                for day in complete_days:
                    ...
                    if (abs((day_date - t).days) < 30 and  # Within month
                        day_date.dayofweek == t.dayofweek):  # Same day of week
