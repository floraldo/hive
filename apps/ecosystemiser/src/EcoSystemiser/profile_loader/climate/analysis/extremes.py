...
def find_extreme_events(ds: xr.Dataset,
                        ...
                        min_duration_hours: int = 24) -> List[Dict]:
    """
...
def identify_heat_waves(ds: xr.Dataset,
                        ...
                        night_relief_threshold: float = 20) -> List[Dict]:
    """
    ...
    heat_wave_days = (
        (daily_max > temp_threshold) & 
        (daily_min > night_relief_threshold)
    )
...
def summarize_extremes(ds: xr.Dataset) -> Dict:
    ...
    # Wind extremes
    if 'wind_speed' in ds:
        wind_extremes = find_extreme_events(ds, 'wind_speed', 
    ...
    if 'precip' in ds:
        heavy_rain = find_extreme_events(ds, 'precip',
