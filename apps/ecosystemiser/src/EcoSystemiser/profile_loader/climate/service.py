...
from EcoSystemiser.profile_loader.data_models import ClimateRequest, ClimateResponse, CANONICAL_VARIABLES
...
from EcoSystemiser.profile_loader.processing.resampling import resample_dataset
from EcoSystemiser.profile_loader.processing.validation import apply_quality_control
from EcoSystemiser.profile_loader.shared.timezone import TimezoneHandler
from EcoSystemiser.profile_loader.analysis.building_science import derive_building_variables
from EcoSystemiser.profile_loader.analysis.synthetic.bootstrap import multivariate_block_bootstrap
from EcoSystemiser.profile_loader.analysis.synthetic.copula import copula_synthetic_generation
from EcoSystemiser.profile_loader.analysis.synthetic.tmy import TMYGenerator, TMYMethod
from EcoSystemiser.profile_loader.analysis.statistics import describe
from EcoSystemiser.profile_loader.subsets import apply_subset
from EcoSystemiser.profile_loader.cache import cache_key_from_request, save_parquet_and_manifest, load_from_cache
from EcoSystemiser.profile_loader.manifest import build_manifest
from EcoSystemiser.settings import get_settings
from EcoSystemiser.profile_loader.adapters.factory import get_adapter, list_available_adapters, get_enabled_adapters
from EcoSystemiser.profile_loader.processing.pipeline import ProcessingPipeline
from EcoSystemiser.shared.service import BaseProfileService, ProfileServiceError, ProfileValidationError
from EcoSystemiser.shared.models import BaseProfileRequest
...
class LocationResolver:
    ...
    def get_adapter_coverage_score(self, lat: float, lon: float, adapter_name: str) -> float:
        ...
        coverage_regions = {
            'nasa_power': {
                'global': True,
                'score': 0.8  # Good global coverage
            },
            'era5': {
                'global': True,
                'score': 0.9  # Excellent global coverage
            },
            'meteostat': {
                'regions': [
...
class ClimateService(BaseProfileService):
    ...
    def _select_best_adapter(self, lat: float, lon: float, requested_source: Optional[str] = None) -> str:
        ...
        logger.info(f"Selected adapter {best_adapter} for location ({lat:.3f}, {lon:.3f}) "
    ...
    def get_source_coverage(self, source: str) -> Dict[str, Any]:
        ...
        from EcoSystemiser.profile_loader.adapters.capabilities import get_adapter_capabilities
    ...
    async def shutdown(self):
        ...
        from EcoSystemiser.profile_loader.adapters.factory import cleanup
        cleanup()
