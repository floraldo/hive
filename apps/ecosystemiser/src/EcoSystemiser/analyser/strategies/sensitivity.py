...
class SensitivityAnalysis(BaseAnalysis):
        ...
        for param_name, indices in sensitivity_indices.items():
            ...
            if sensitivities:
                ...
                influential.append({
                    ...
                    'most_affected_kpi': max(indices.items(), key=lambda x: abs(x[1]) if isinstance(x[1], (int, float)) else 0)[0]
                })
    ...
    def _find_optimal_configurations(self, all_results: List[Dict]) -> Dict[str, Dict]:
        ...
        if cost_results and renewable_results:
            ...
            balanced_results = [r for r in successful if all(k in r.get('kpis', {}) for k in ['total_cost', 'renewable_fraction'])]
            if balanced_results:
