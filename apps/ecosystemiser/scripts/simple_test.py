...
def test_direct_adapter() -> None:
    ...
    class MockAdapter(BaseAdapter):
        ...
        def get_capabilities(cls) -> AdapterCapabilities:
            return AdapterCapabilities(adapter_name="mock", temporal_coverage=TemporalCoverage(start_date="2000-01-01", end_date="2023-12-31", historical_depth_years=23, forecast_horizon_days=0, near_real_time_lag_hours=24), spatial_coverage=SpatialCoverage(global_coverage=True, regional_limitations=[], resolution_degrees=0.5, altitude_range_m=(-500, 9000)), data_frequency=DataFrequency(available_resolutions=["1H", "D"], native_resolution="1H", aggregation_methods=["mean", "sum", "min", "max"]), supported_variables=list(CANONICAL_VARIABLES.keys()), data_quality=QualityFeatures(has_quality_flags=False, has_uncertainty=False, has_validation=False, missing_data_threshold=0.1), authentication=AuthType.NONE, rate_limits=RateLimits(requests_per_minute=60, requests_per_day=10000, concurrent_requests=10, requires_throttling=False), performance={"typical_latency_ms": 100, "max_chunk_size_days": 365, "supports_caching": True})
        ...
        async def fetch_async(self, lat, lon, variables, period, resolution="1H") -> None:
            ...
            ds = xr.Dataset(data, coords={"time": times, "latitude": lat, "longitude": lon}, attrs={"source": "mock", "description": "Mock climate data for testing", "created": datetime.now().isoformat()})
    ...
    try:
        ...
        async def test_fetch_async():
            ds = await adapter.fetch_async(lat=51.5, lon=-0.1, variables=["temp_air", "ghi", "wind_speed"], period={"start": "2023-01-01", "end": "2023-01-07"}, resolution="1H")
            return ds
...
def test_service_with_mock() -> None:
    ...
    try:
        ...
        request = ClimateRequest(location=(51.5, -0.1), variables=["temp_air"], period={"start": "2023-01-01", "end": "2023-01-02"}, source="nasa_power", mode="observed")
