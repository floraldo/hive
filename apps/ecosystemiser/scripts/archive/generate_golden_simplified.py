...
def run_original_systemiser() -> None:
    ...
    result = subprocess.run(cmd, cwd=str(systemiser_dir), capture_output=True, text=True)
...
def extract_minimal_components(original_data) -> None:
    ...
    - Battery
    """
    minimal_data = {"storage_levels": {}, "flows": [], "metadata": original_data.get("metadata", {})}
    ...
    if "flows" in original_data:
        for flow in original_data["flows"]:
            ...
            involves_minimal = any(comp in from_comp for comp in minimal_components) or any(
                comp in to_comp for comp in minimal_components
            )
