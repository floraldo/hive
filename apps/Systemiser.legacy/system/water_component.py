...
class WaterComponent(Component):
    ...
    def get_technical_results(self):
        ...
        technical_results_df = pd.DataFrame(list(self.technical_results.items()), 
                                          columns=['Metric', 'Value']).round(2)
    ...
    def get_water_loss(self):
        return np.array([flow['value'] for flow in self.flows['sink'].values() 
                        if 'loss' in flow['name']]) / 1e3
    ...
    def get_internal_water_changes(self):
        ...
        for flow_type in ['source', 'sink']:
            for flow in self.flows.get(flow_type, {}).values():
                if flow['type'] == 'water':
                    flow_value = np.array(flow['value'].value if isinstance(flow['value'], cp.Variable) 
                                        else flow['value']) / 1e3
    ...
    def set_constraints(self):
        for t in range(self.N):
            ...
            input_flows = cp.sum([-flow['value'][t] for flow in self.flows['input'].values() 
                                if isinstance(flow['value'], cp.Variable)])
            output_flows = cp.sum([flow['value'][t] for flow in self.flows['output'].values() 
                                 if isinstance(flow['value'], cp.Variable)])
            source_flows = cp.sum([flow['value'][t] for flow in self.flows['source'].values() 
                                 if isinstance(flow['value'], cp.Variable)])
            sink_flows = cp.sum([-flow['value'][t] for flow in self.flows['sink'].values() 
                                if isinstance(flow['value'], cp.Variable)])
