                    ...
                    for date_key, value in values_dict.items():
                        try:
                            ...
                            if len(date_key) == 10:  # YYYYMMDDHH
                                ...
                                self.logger.warning(f"Received daily date key '{date_key}' for param '{param_api}'. Expected hourly (YYYYMMDDHH)")
                                continue
                            else:
                            ...
                            self.logger.warning(f"Skipping value '{value}' for date '{date_key}' in param '{param_api}' due to ValueError: {e}")
                            continue
