        ...
        for flow_direction in self.flows:
            for flow_name in self.flows[flow_direction]:
                self.flows[flow_direction][flow_name]['value'] = (
                    ...
                    if method == 'optimization' else np.zeros(N)
                )
        
        # Initialize storage variables if applicable
        if self.type == 'storage':
            self.variables['E'] = (
                cp.Variable(N, name=f"{self.name}_E")
                if method == 'optimization' else np.zeros(N)
            )
    ...
    def _add_storage_constraints(self, constraints):
        ...
        if E is not None:
            ...
            constraints.extend([
                ...
                E <= self.E_max,
                P_charge <= self.F_max,
                P_discharge <= self.F_max
            ])
            
            for t in range(1, self.N):
                constraints.append(
                    E[t] == E[t-1] + self.eta * (P_charge[t-1] - P_discharge[t-1])
                )
...
class WaterPond(StorageComponent):
    ...
    def set_constraints(self):
        ...
        if E is not None:
            ...
            constraints.extend([
                W_infiltration == self.infiltration_rate * E
            ])
        return constraints
