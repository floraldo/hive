...
class HeatPump(Component):
    def __init__(self, name, cop, eta, n, P_max=None, economic=None, environmental=None):
        ...
        self.eta = eta
        self.medium = "heat"
        self.flows['source']['P_heatsource'] = {'type': 'heat',
                                                'value': cp.Variable(n, nonneg=True, name=f'{name}_P_heatsource')}
        self.flows['sink']['P_loss'] = {'type': 'electricity', 'value': cp.Variable(n, nonneg=True, name=f'{name}_P_loss')}
        self.flows['sink']['P_pump'] = {'type': 'electricity', 'value': cp.Variable(n, nonneg=True, name=f'{name}_P_pump')}

    def set_constraints(self):
        for t in range(self.N):
            input_flows = cp.sum(
                [flow['value'][t] for direction in self.flows for flow_name, flow in self.flows[direction].items() if
                 direction == 'input'])
