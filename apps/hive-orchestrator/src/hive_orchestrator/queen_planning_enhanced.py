...
from .core.db.planning_integration import planning_integration, async_planning_integration
...
class QueenPlanningEnhanced(QueenLite):
        ...
        try:
            ...
            if planned_subtasks:
                ...
                for task in planned_subtasks[:3]:  # Log first 3 for visibility
                    ...
                    logger.info(f"  📋 {task['id']}: {task['title']} (phase: {plan_ctx.get('workflow_phase', 'unknown')})")

            logger.info(f"[ENHANCED] Processing {len(all_tasks)} tasks ({len(planned_subtasks)} planned + {len(regular_tasks)} regular)")
    ...
    def print_status_enhanced(self):
        ...
        logger.info(f"[PLANNING] Plans triggered: {stats['plans_triggered']}, "
            ...
            with planning_integration._get_connection() as conn:
                ...
                if active_plans:
                    ...
                    for plan_id, status in active_plans:
                        ...
                        logger.info(f"  📋 {plan_id}: {status} ({completion.get('completion_percentage', 0):.1f}% complete)")
