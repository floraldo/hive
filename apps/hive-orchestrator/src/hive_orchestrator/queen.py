...
class QueenLite:
    ...
    def get_next_command_from_workflow(self, task: Dict[str, Any]) -> str | None:
    ...
    def _format_command(self, template: str, task: Dict[str, Any], run_id: str | None = None) -> str:
    ...
    def _advance_task_phase(self, task: Dict[str, Any], success: bool = True) -> str | None:
    ...
    def _handle_worker_success(
        ...
        if not run_data:
            self.log.warning(
                f"Worker for {task_id} finished successfully but no run data was found for run_id: {run_id}"
            )
    ...
    async def process_review_tasks_async(self) -> None:
        ...
        for task in review_tasks:
            ...
            self.log.info(
                f"  Use: 'hive complete-review {task_id} --decision [approve|reject|rework]' to complete review"
            )
