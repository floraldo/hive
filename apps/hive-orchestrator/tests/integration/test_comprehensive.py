...
class HiveTestSuite:
    ...
    def test_error_handling(self) -> bool:
        ...
        env["PYTHONPATH"] = orchestrator_src
        try:
            process = subprocess.run(cmd, check=False, stdin=subprocess.DEVNULL, capture_output=True, text=True, env=env, timeout=10)
    ...
    def test_queen_runner(self) -> bool:
        ...
        if not queen_script.exists():
            ...
            return False
        try:
            result = subprocess.run([sys.executable, str(queen_script), "--help"], check=False, capture_output=True, text=True, timeout=5)
