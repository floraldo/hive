...
class CitadelGuardian:
    ...
    async def _scan_for_violations_async(
        self, 
        changed_files: list[str], 
        commit_sha: str
    ) -> list[CitadelViolation]:
        ...
        try:
            ...
            violations.append(CitadelViolation(
        ...
        try:
            if self.data_layer:
                ...
                if compliance_metrics:
                    ...
                    comprehensive_scans = [m for m in compliance_metrics 
                                         if m.tags.get("scan_type") == "comprehensive"]
            ...
            for component, comp_violations in component_violations.items():
                ...
                for violation in comp_violations[:3]:  # Top 3 violations per component
                    if violation.suggested_fix:
                        recommendations["fixes"].append(
                            f"{component}: {violation.suggested_fix}"
                        )
...
"""
    ...
    async def create_epic_for_remaining_gaps_async(self) -> dict[str, Any]:
        ...
        try:
            ...
            critical_components = [
                scorecard for scorecard in cert_readiness.component_scorecards
...
"""
            ...
            return {
                ...
                "summary": {
                    ...
                    "target_completion": (datetime.utcnow() + timedelta(days=cert_readiness.estimated_effort_days)).isoformat(),
