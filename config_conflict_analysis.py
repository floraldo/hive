                    ...
                    for line in lines:
                        if line.strip().startswith('[tool.ruff'):
                            ...
                            if str(vscode_line_length) != pyproject_line_length:
                                print(f"  ⚠️  CONFLICT: Line length mismatch!")
                                print(f"    VS Code: {vscode_line_length}")
                                print(f"    PyProject: {pyproject_line_length}")
                            else:
                                print(f"  ✅ Line length settings match")
                            break
                except Exception as e:
...
def check_file_watchers():
    ...
    try:
        ...
        result = subprocess.run("ps aux | grep -E '(watch|monitor|inotify)' | grep -v grep", 
        ...
        result = subprocess.run("ps aux | grep -i 'visual studio code' | grep -E '(watch|monitor)' | grep -v grep", 
                              shell=True, capture_output=True, text=True)
