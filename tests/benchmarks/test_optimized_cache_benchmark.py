...
class OptimizedCacheBenchmark:
    ...
    def __init__(self):
        self.test_data = {
            "tiny": {"status": "ok"},
            "small": {"key": "value", "number": 42, "active": True},
            "medium": {
                "data": "x" * 1000,
                "items": list(range(50)),
                "metadata": {"created": "2025-09-29", "version": "v4.2"},
            },
        }
    ...
    async def benchmark_optimized_cache_client(self) -> list[BenchmarkResult]:
        ...
        config = CacheConfig(
            ...
            max_connections=20,
            response_timeout=1.0,  # Reduced from 5s to 1s
            circuit_breaker_enabled=False,  # Disabled for pure performance test
            compression_enabled=False,  # Disabled for latency test
            serialization_format="msgpack",  # Fastest serialization
        )
