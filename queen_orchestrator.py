...
class QueenOrchestrator:
    ...
    def __init__(self):
        ...
        # Track active spawns (task_id -> process)
    ...
    def spawn_worker(self, task: Dict[str, Any], worker: str, phase: Phase) -> Optional[subprocess.Popen]:
    ...
    def check_worker_result(self, task_id: str) -> Optional[Dict[str, Any]]:
        """Check for latest worker result"""
...
"""
    ...
    def process_queue(self):
        ...
        for task_id in queue[:]:
            ...
            process = self.spawn_worker(task, worker, Phase.PLAN)
            if process:
    ...
    def monitor_workers(self):
        ...
        for task_id, process in list(self.active_workers.items()):
            ...
            if poll is not None:
                ...
                # Check result
                result = self.check_worker_result(task_id)
                if result:
                    ...
                    if task and task.get("status") == "in_progress":
                        ...
                        # Spawn next phase
                        new_process = self.spawn_worker(task, worker, next_phase)
                        if new_process:
                            self.active_workers[task_id] = new_process
                            continue
