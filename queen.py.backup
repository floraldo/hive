...
class QueenLite:
    ...
    def process_queued_tasks(self):
        """Process tasks in queued status by starting their workflows"""
    ...
    def process_workflow_tasks(self):
        """Process queued tasks using workflow definitions"""
        queued_tasks = hive_core_db.get_queued_tasks()
        if not queued_tasks:
            return
        ...
        for task in queued_tasks:
            ...
            try:
                ...
                self.log.info(f"Executed task {task_id} phase '{current_phase}' (PID: {process.pid})")
